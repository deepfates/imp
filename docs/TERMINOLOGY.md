...
Dachshund should read like an Elixir system, not a Python glossary with new
...
| Term | Meaning |
| --- | --- |
| Dachshund | The project name and public API for declarative self-improving programs in Elixir. |
...
| LM | A configured language-model client implementing the `Dachshund.LM` behaviour. |
...
Use `Dachshund` in beginner docs, application snippets, and operator-facing code:
...
Use `Dachshund.*` when naming implementation modules, behaviours, and advanced
...
| Avoid | Prefer |
| --- | --- |
| Elixir port | Dachshund, or Elixir-native system |
