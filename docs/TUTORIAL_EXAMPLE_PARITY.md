...
| RAG and multi-hop RAG | [API Guide: Retrieval-Augmented Programs](API_GUIDE.md#retrieval-augmented-programs) with `Imp.rag/3` and `hops:` | `mix test test/public_surface_test.exs test/integration/local_service_e2e_test.exs` | Supported local and integration example |
...
| Program of Thought and CodeAct | [API Guide: Which Program Shape?](API_GUIDE.md#which-program-shape) with `Imp.program_of_thought/2` and `Imp.code_act/3` | `mix test test/program_of_thought_fidelity_test.exs` | Supported sandboxed-code example |
