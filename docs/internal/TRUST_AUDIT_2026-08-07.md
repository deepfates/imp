...
5. **Two broken snippets in docs/LEARNING_PATH.md** (both fail loudly at construction): the HTTP retriever `response_mapper` example is arity-2 where the schema requires `{:fun, 1}` (`http.ex:45`), and `Imp.Retrievers.Databricks.new/3` is documented but only `new/2` exists (`http.ex:449`). Neither touches benchmark paths.
