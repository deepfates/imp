...
| Finetuning / BetterTogether / GRPO | DSPy paper list and docs include finetuning plus prompt optimization. | BootstrapFinetune has a protocol-compatible lifecycle plus pinned local MLX SFT effectiveness and fused save/load rebinding. BetterTogether can continue from that weight step into prompt optimization. GRPO remains an external reinforcement-trainer orchestration boundary; Imp has no local GRPO update engine. | `mix protocol.training.check`, `mix imp.benchmark.local_mlx`, `mix benchmark.optimizer_lift.check` | Retain paid-provider, BetterTogether parity, and actual GRPO weight updates as distinct gaps until their own matched campaigns pass. |
