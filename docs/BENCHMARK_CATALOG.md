...
| GEPA paper replication | GEPA artifact repo covers AIMEBench, HotpotQABench, hoverBench, IFBench, LiveBenchMathBench, and Papillon. | Artifact contract implemented; real paid/source-checkout campaign rows are required before dominance claims. | `mix benchmark.gepa_replication.check` | Run fresh DSEx-vs-DSPy GEPA campaigns with exact model, budget, cost, seed, and split-gap metadata. |
