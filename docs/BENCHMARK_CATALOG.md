...
| RAG/retrieval | DSP and DSPy papers emphasize retrieval + generation for knowledge-intensive QA. | Provider-free deterministic one-shot RAG, multi-hop RAG, retriever protocol gates, and a 15/15 matched-live Haiku artifact exist. | `mix benchmark.rag_tool_agent.check`, `mix imp.benchmark.rag_tool_agent --live ...`, `mix protocol.retriever.check` | Scale retrieval corpora only for research-tier quality claims; the bounded production-behavior claim is complete. |
