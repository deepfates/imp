...
| RLM recursive control | DSPy RLM docs and paper position recursive controller loops as long-context inference over variable space, distinct from RAG. | Provider-free DSEx-vs-DSPy RLM benchmark lane exists over HotPotQA-shaped long-context fixture rows, plus deterministic public-surface, budget, recursion, lazy-load, extract, and redaction coverage. | `mix benchmark.rlm.check`, `mix test test/rlm_test.exs`, `mix integration.check` | Add live sampled long-context RLM campaigns only when making model-quality claims beyond provider-free operational parity. |
...
| Optimizer lift | DSPy optimizer docs cover few-shot bootstrapping, instruction/demo search, MIPROv2, GEPA, and finetuning workflows. | Provider-free lift implemented, including DSEx-vs-DSPy optimizer rows plus natural classification, QA, retrieval, and instruction-following lanes. | `mix benchmark.optimizer_lift.check` | Scale the natural lanes to larger sampled datasets when release policy requires model-quality evidence beyond provider-free product proof. |
| GEPA paper replication | GEPA artifact repo covers AIMEBench, HotpotQABench, hoverBench, IFBench, LiveBenchMathBench, and Papillon. | Artifact contract, dataset exporter, and DSEx-side campaign row producer exist; real paid/source-checkout campaign rows are required before dominance claims. | `mix benchmark.gepa_replication.check`, `mix dsex.benchmark.gepa_dataset`, `mix dsex.benchmark.gepa_campaign` | Finish HoVer retrieval-corpus parity and run fresh DSEx-vs-DSPy GEPA campaigns with exact model, budget, cost, seed, and split-gap metadata. |
...
| HoVer claim verification | MIPRO and GEPA benchmark lineage includes HoVer multi-hop verification. | GEPA exporter and metric adapter now use the upstream `retrieved_docs` contract; source-exact BM25/wiki corpus parity remains open. | `mix dsex.benchmark.gepa_dataset`, `mix dsex.benchmark.gepa_campaign` | Pin or reproduce the upstream wiki.abstracts.2017 BM25 retriever before claiming source-exact HoVer campaign parity. |
