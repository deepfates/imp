...
| Typed fields, descriptions, constraints, JSON schema | Implemented | `DSEx.Signature.Field`, `DSEx.Schema` | `test/schema_constraints_test.exs`, `test/production_adapter_persistence_test.exs` | Live JSON gate exercises provider schema-constrained output | `docs/API_GUIDE.md`, `docs/ARCHITECTURE.md` | Keep and extend only when provider needs require it |
...
| Concept | DSEx status | DSEx surface | Deterministic tests | Integration/live proof | Docs | Release decision |
| --- | --- | --- | --- | --- | --- | --- |
| LM behaviour and injectable clients | Implemented | `DSEx.LM`, `DSEx.Clients.HTTPLM`, provider constructors | `test/completion_surface_test.exs`, `test/production_hardening_test.exs` | `LIVE_PROVIDER=1 mix live.check` | `docs/API_GUIDE.md`, `docs/PRODUCTION_OPERATIONS.md` | Keep behaviour-first design |
| OpenAI-compatible chat completions | Implemented | `DSEx.Clients.OpenAI`, `DSEx.Clients.HTTPLM` | `test/completion_surface_test.exs`, `test/provider_tool_call_test.exs` | Live basic prediction, JSON, CoT, streaming, ReActV2 | `README.md`, `docs/API_GUIDE.md` | Keep OpenAI-compatible surface as provider baseline |
...
| Streaming | Implemented | `DSEx.Streaming`, `DSEx.Clients.HTTPLM.stream/3` | `test/provider_streaming_test.exs`, `test/completion_surface_test.exs` | Live provider streaming test | `docs/API_GUIDE.md`, `docs/PRODUCTION_OPERATIONS.md` | Keep streaming as enumerable interface |
...
| MCP HTTP, stdio, Streamable HTTP transports | Implemented | `DSEx.MCP.HTTP`, `DSEx.MCP.Stdio`, `DSEx.MCP.StreamableHTTP` | `test/mcp_import_test.exs` | Local MCP integration gate required: `de-wrnz` | `docs/API_GUIDE.md`, `docs/ARCHITECTURE.md` | Keep, but prove with local servers before V3 |
...
| External HTTP retrievers | Implemented | `DSEx.Retrievers.HTTP`, `Weaviate`, `Databricks` | `test/external_retriever_test.exs` | Local HTTP integration gate required: `de-wrnz` | `docs/API_GUIDE.md`, `docs/ARCHITECTURE.md` | Keep as payload-contract clients; live external retriever gate optional |
...
| Concept | DSEx status | DSEx surface | Deterministic tests | Integration/live proof | Docs | Release decision |
| --- | --- | --- | --- | --- | --- | --- |
| Evaluation loop | Implemented | `DSEx.Evaluate` | `test/dsex_test.exs`, `test/public_surface_test.exs` | No live proof required for V3 | `docs/API_GUIDE.md`, `livebooks/02_evaluate_and_optimize.livemd` | Metric contract upgrade owned by `de-2iou` |
| Metrics | Release blocker | `DSEx.Metrics`, metric functions accepted by optimizers | `test/dsex_test.exs`, optimizer suites | No live proof required for V3 | `docs/API_GUIDE.md` | Expand bool/number/feedback/trace contract under `de-2iou` |
...
| COPRO, MIPROv2, SIMBA, GEPA | Implemented | `DSEx.Optimizer.COPRO`, `MIPROv2`, `SIMBA`, `GEPA` | `test/optimizer_behavioral_corpus_test.exs`, `test/v2_benchmark_test.exs` | No live proof required for V3; deterministic reward-encoding controls are the oracle | `docs/V2.md`, `docs/API_GUIDE.md` | Keep, with metric feedback upgrade under `de-2iou` |
...
| BootstrapFinetune and GRPO provider jobs | Release blocker | `DSEx.Optimizer.BootstrapFinetune`, `DSEx.Optimizer.GRPO`, `DSEx.Clients.Trainer` | `test/provider_training_lifecycle_test.exs`, `test/completion_surface_test.exs` | Optional live training gate needed if production claim remains: `de-i8cc`, `de-ld5r`, `de-wrnz` | `docs/API_GUIDE.md`, `docs/ARCHITECTURE.md` | Resolve stubs and claims under `de-ld5r` |
...
| Concept | DSEx status | DSEx surface | Deterministic tests | Integration/live proof | Docs | Release decision |
| --- | --- | --- | --- | --- | --- | --- |
| Save/load | Implemented | `DSEx.Saving`, program `dump/1` paths | `test/production_adapter_persistence_test.exs`, `test/production_hardening_test.exs` | Save/load/rebind/deployed-call integration gate required: `de-wrnz` | `docs/API_GUIDE.md`, `docs/ARCHITECTURE.md` | Keep with explicit credential rebinding |
| Secret redaction | Implemented | `DSEx.Redaction`, trace call sites | `test/agent_runtime_test.exs`, `test/production_hardening_test.exs` | Observability redaction tests required under `de-x02m` | `docs/PRODUCTION_OPERATIONS.md` | Keep as non-negotiable security invariant |
| Telemetry and observability | Release blocker | `DSEx.Telemetry`, runtime call sites | `test/production_hardening_test.exs` covers existing hooks | Full event model required under `de-x02m` | `docs/PRODUCTION_OPERATIONS.md` | Complete under `de-x02m` |
