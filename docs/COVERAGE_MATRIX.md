...
| Stateful live aliases | Implemented | `mix live.training.check`, `mix live.retriever.check`, `mix live.mcp.check` | Gate tests under `test/live_*` | Training proves provider-compatible submit/refresh over real HTTP transport; retriever proves Weaviate/Databricks-compatible HTTP retrieval; MCP proves JSON-RPC HTTP, Streamable HTTP, and trusted stdio clients | `docs/PRODUCTION_OPERATIONS.md`, `docs/RELEASE_CRITERIA.md` | Do not claim paid/external-service proof unless the matching alias is configured against that service |
