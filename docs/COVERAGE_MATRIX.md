...
| DSPy parity lane | Implemented | `mix benchmark.parity.check`, `mix benchmark.parity.full`, `scripts/dspy_parity_runner.py` | Alias contract test | Smoke lane proves wiring; full lane runs DSEx and Python DSPy over full canonical GSM8K/HotPotQA splits with the same live model endpoint | `docs/BENCHMARK_TRUTH.md` | Full-lane artifacts required before claiming original-library parity |
