...
if [ -z "$PYTHON" ]; then
  echo "Imp parity requires Python 3.12+ for the locked comparator; no python3.12+ executable found." >&2
  exit 1
fi
...
if sys.version_info < (3, 12):
    raise SystemExit(
        f"Imp parity requires Python 3.12+ for the locked comparator; found {sys.version.split()[0]}"
    )
PY
...
PY
  then
    if [ "$VENV_DIR" = "tmp/dspy-parity-venv" ] || [ "$RECREATE" = "1" ]; then
      ...
      echo "$VENV_DIR uses Python older than 3.12. Set IMP_DSPY_RECREATE=1 to rebuild it." >&2
      exit 1
    fi
  fi
fi
