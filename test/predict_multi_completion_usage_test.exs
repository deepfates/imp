defmodule PredictMultiCompletionUsageTest do
  ...
  describe "n= multi-completion loudness" do
    test "an LM that ignores :n and returns a single output is a loud error, not one quiet completion" do
      lm = fn _messages, _opts -> {:ok, %{answer: "only one"}} end
      program = Imp.Predict.Predict.new("question -> answer", lm: lm, config: [n: 3])
    ...
    test "an invalid :n is a loud error" do
      lm = fn _messages, _opts -> {:ok, %{answer: "x"}} end
      program = Imp.Predict.Predict.new("question -> answer", lm: lm, config: [n: 0])
    ...
    test "n > 1 with unset or near-zero temperature bumps to 0.7; an explicit temperature survives" do
      ...
      program = Imp.Predict.Predict.new("question -> answer", lm: lm, config: [n: 2])

      assert {:ok, _prediction} = Imp.Predict.Predict.call(program, %{question: "q"})
      assert_received {:lm_call, _messages, opts}
      assert opts[:temperature] == 0.7

      hot =
        Imp.Predict.Predict.new("question -> answer", lm: lm, config: [n: 2, temperature: 1.0])

      assert {:ok, _prediction} = Imp.Predict.Predict.call(hot, %{question: "q"})
      ...
      assert {:ok, _prediction} = Imp.Predict.Predict.call(single, %{question: "q"})
  ...
  describe "per-call config (call/3)" do
    test "call-time config reaches the LM without mutating the program" do
      ...
      program = Imp.Predict.Predict.new("question -> answer", lm: lm, config: [temperature: 0.3])

      assert {:ok, _prediction} =
               Imp.Predict.Predict.call(program, %{question: "q"}, temperature: 0.9)
