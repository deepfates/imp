...
defmodule MatchedIFBenchV3Imp.Observer do
  ...
  def register_budget(pid, seed, arm, ceiling) do
    Agent.update(pid, fn state ->
      ...
      budget = %{
        ceiling: ceiling,
        counts: MatchedGepaMiproIFBenchV3.CallBudget.zero(),
        refusals: []
      }
    ...
    result =
      Agent.get_and_update(pid, fn state ->
        ...
        try do
          ...
          projected =
            MatchedGepaMiproIFBenchV3.CallBudget.reserve!(
              budget.counts,
              budget.ceiling,
              role
            )
...
defmodule MatchedIFBenchV3Imp.ObservedLM do
  ...
  def generate(lm, messages, opts) do
    ...
    with :ok <-
         ...
         :ok <-
           MatchedIFBenchV3Imp.Observer.reserve_call!(lm.observer, lm.seed, lm.arm, lm.role) do
      MatchedIFBenchV3Imp.Observer.message(lm.observer, lm.role, messages)
      result = dispatch(lm, messages, opts)
      MatchedIFBenchV3Imp.Observer.response(lm.observer, lm.role, result)
  ...
  defp validate_response(lm, result) do
    evidence = MatchedGepaMiproIFBenchV3.ResponseEvidence.from_result!(result)
    expected = lm.expected_model

    if is_number(evidence.gateway_reported_cost) and evidence.gateway_reported_cost >= 0 do
      MatchedIFBenchV3Imp.Observer.reconcile_cost!(lm.observer, evidence.gateway_reported_cost)
    end

    case MatchedGepaMiproIFBenchV3.ResponseEvidence.validate_contract(evidence, expected) do
      :ok ->
        :ok
...
defmodule MatchedIFBenchV3Imp.Runner do
  alias Imp.Optimizer.{Artifact, GEPA, MIPROv2, Report}
  alias MatchedIFBenchV3Imp.{ObservedLM, Observer}

  @manifest Path.expand("contract.json", __DIR__)
  @output Path.expand(
            System.get_env(
              "IMP_MATCHED_IFBENCH_V3_OUTPUT",
              "../../tmp/matched_gepa_mipro_ifbench_v3/imp-result.json"
            ),
            __DIR__
          )
  ...
  def run do
    manifest = MatchedGepaMiproIFBenchV3.Contract.load_optimization!(@manifest)
    ...
    try do
      ...
      {sealed, held_out} =
        MatchedGepaMiproIFBenchV3.TwoPhase.seal_then_load_held_out!(
          work_items,
  ...
  defp wait_for_peer_selection!(manifest) do
    peer =
      System.get_env(
        "IMP_MATCHED_IFBENCH_V3_UPSTREAM_SELECTION",
        Path.expand(
          "../../tmp/matched_gepa_mipro_ifbench_v3/upstream-result.json.selection-sealed.json",
          __DIR__
        )
      )
  ...
  defp evaluate(program, rows, observer, seed, arm, phase, expected_model) do
    ...
    Enum.map(rows, fn row ->
      ...
      evidences =
        Enum.map(responses, &MatchedGepaMiproIFBenchV3.ResponseEvidence.from_result!(&1.result))
  ...
  defp validate_response_ledger!(before, after_snapshot, manifest) do
    ...
    Enum.each(responses, fn entry ->
      response = MatchedGepaMiproIFBenchV3.ResponseEvidence.from_result!(entry.result)
  ...
  defp validate_transport_evidence!(evidence, expected, context) do
    ...
    unless evidence.model in [configured, expected["imp"]] and
             ...
             MatchedGepaMiproIFBenchV3.ResponseEvidence.costs_reconcile?(
  ...
  defp source_commits!(manifest, clean?, expected \\ require_expected_launch_commit!()) do
    ...
    commits =
      if clean?,
        do: MatchedGepaMiproIFBenchV3.SourceIdentity.capture_clean!(root, pinned),
        else: MatchedGepaMiproIFBenchV3.SourceIdentity.current(root, pinned)

    if expected == commits["imp"],
      do: commits,
      else: raise("v3 launch commit drift: #{commits["imp"]} != #{expected}")
  end
  ...
  defp require_expected_launch_commit! do
    case System.get_env("MATCHED_IFBENCH_V3_EXPECTED_COMMIT") do
      value when is_binary(value) and byte_size(value) == 40 -> value
      _ -> raise "MATCHED_IFBENCH_V3_EXPECTED_COMMIT is required as a full commit"
    end
  end

  defp stopped_binding_fields(
         launch_commit \\ System.get_env("MATCHED_IFBENCH_V3_EXPECTED_COMMIT") || "unavailable"
       ) do
    manifest = @manifest |> File.read!() |> Jason.decode!()
  ...
  defp rescue_accounting(snapshot) do
    MatchedIFBenchV3Imp.StopAccounting.normalize(
  ...
  defp empty_rescue_accounting, do: MatchedIFBenchV3Imp.StopAccounting.empty()
...
unless System.get_env("IMP_MATCHED_IFBENCH_V3_LOAD_ONLY") == "1" do
  MatchedIFBenchV3Imp.Runner.run()
end
