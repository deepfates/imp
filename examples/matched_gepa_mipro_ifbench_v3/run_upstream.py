...
def require_expected_launch_commit() -> str:
    ...
    if actual != expected:
        raise LaunchAdmissionError(f"v3 launch commit drift: {actual} != {expected}")
    return expected
...
def load_authenticated_runtime() -> Any:
    ...
    global v1, _patched_dspy_gepa, _v1_source_commits, _v1_atomic_write, _v1_compile_arm
    ...
    runtime.MANIFEST_PATH = V3_MANIFEST
    runtime.OUTPUT = Path(
        os.environ.get(
            "UPSTREAM_MATCHED_IFBENCH_V3_OUTPUT",
            IMP_ROOT / "tmp" / "matched_gepa_mipro_ifbench_v3" / "upstream-result.json",
        )
    )
