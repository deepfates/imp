...
def treatment_preservation_gate(contract: dict[str, Any]) -> dict[str, Any]:
    v2_path = (HERE / contract["predecessor"]["contract_path"]).resolve()
    require(
        sha256_file(v2_path) == contract["predecessor"]["contract_sha256"],
        "terminal v2 contract drift",
    )
    ...
    for key in (
        ...
        require(contract[key] == v2[key], f"v3 changed preserved treatment field {key}")
    require(
        ...
        "v3 launch state drift",
    )
    return {
        "terminal_v2_contract_sha256": contract["predecessor"]["contract_sha256"],
        "preserved_fields": 11,
        "sole_semantic_delta": "GEPA failure-cardinality compatibility layer",
    }
...
def runner_gate(
    ...
    spec = importlib.util.spec_from_file_location(
        "matched_ifbench_v3_upstream", HERE / "run_upstream.py"
    )
    require(
        spec is not None and spec.loader is not None, "cannot load v3 upstream runner"
    )
