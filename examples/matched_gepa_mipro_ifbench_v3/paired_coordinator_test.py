...
class PairedCoordinatorTest(unittest.TestCase):
    ...
    def test_upstream_rescue_distinguishes_failed_and_in_flight_transports(self) -> None:
        manifest = json.loads((HERE / "contract.json").read_text())
        accounting = {
            "call_budgets": [
                {
    ...
    def test_upstream_direct_entry_requires_launch_commit_before_argument_parse(
        ...
        env = dict(os.environ)
        env.pop("MATCHED_IFBENCH_V3_EXPECTED_COMMIT", None)
    ...
    def test_upstream_stop_writer_binds_context_and_normalizes_ledgers(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as root, mock.patch.dict(
            os.environ, {"MATCHED_IFBENCH_V3_EXPECTED_COMMIT": launch_commit}
        ):
    ...
    def test_paired_surface_is_explicit_and_v3_only(self) -> None:
        manifest = json.loads((HERE / "contract.json").read_text())
        expected = {
            "call_budget",
            "consumer_lock",
