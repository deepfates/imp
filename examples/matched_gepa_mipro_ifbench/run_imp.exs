...
defmodule MatchedIFBenchImp.Runner do
  ...
  @output Path.expand(
            System.get_env(
              "IMP_MATCHED_IFBENCH_OUTPUT",
              "../../tmp/matched_gepa_mipro_ifbench/imp-result.json"
            ),
            __DIR__
          )
  ...
  defp wait_for_peer_selection!(manifest) do
    peer =
      System.get_env(
        "IMP_MATCHED_IFBENCH_UPSTREAM_SELECTION",
        Path.expand(
          "../../tmp/matched_gepa_mipro_ifbench/upstream-result.json.selection-sealed.json",
          __DIR__
        )
      )
...
unless System.get_env("IMP_MATCHED_IFBENCH_LOAD_ONLY") == "1" do
