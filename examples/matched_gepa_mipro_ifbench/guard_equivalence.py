...
CHECKS = [
    ...
    ("task_parser", "intentional_difference", ["{:ok, prediction}", "Imp.Prediction.get(prediction, :response)", "{:error, reason}", "length(messages) in 1..2"], ["prediction = program(prompt=row[\"prompt\"])", "len(calls) not in (1, 2)", "except Exception as exc"], "language-native typed adapters; a first-stage parse failure may end the two-stage row after one transport, and is retained with score zero in both"),
    ...
    ("call_ceiling", "identical", ["v014_budget_envelope", "register_budget", "enforce_call_ceiling!", "enforce_combined_ceiling!"], ["gepa_budget_envelope", "register_budget", "validate_call_slice", "validate_combined_ceiling"], "same pinned between-iteration semantic stopper envelope plus separate per-arm and combined operational caps"),
