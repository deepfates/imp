...
defmodule MatchedIFBenchGepa014Imp.Observer do
  ...
  def register_budget(pid, seed, arm, ceiling) do
    Agent.update(pid, fn state ->
      ...
      budget = %{
        ceiling: ceiling,
        counts: MatchedGepaMiproIFBenchGepa014.CallBudget.zero(),
        refusals: []
      }
    ...
    result =
      Agent.get_and_update(pid, fn state ->
        ...
        try do
          ...
          projected =
            MatchedGepaMiproIFBenchGepa014.CallBudget.reserve!(
              budget.counts,
              budget.ceiling,
              role
            )
...
defmodule MatchedIFBenchGepa014Imp.ObservedLM do
  ...
  def generate(lm, messages, opts) do
    ...
    with :ok <-
         ...
         :ok <-
           MatchedIFBenchGepa014Imp.Observer.reserve_call!(lm.observer, lm.seed, lm.arm, lm.role) do
      MatchedIFBenchGepa014Imp.Observer.message(lm.observer, lm.role, messages)
      result = dispatch(lm, messages, opts)
      MatchedIFBenchGepa014Imp.Observer.response(lm.observer, lm.role, result)
  ...
  defp validate_response(lm, result) do
    evidence = MatchedGepaMiproIFBenchGepa014.ResponseEvidence.from_result!(result)
    expected = lm.expected_model

    if is_number(evidence.gateway_reported_cost) and evidence.gateway_reported_cost >= 0 do
      MatchedIFBenchGepa014Imp.Observer.reconcile_cost!(
        lm.observer,
        evidence.gateway_reported_cost
      )
    end

    case MatchedGepaMiproIFBenchGepa014.ResponseEvidence.validate_contract(evidence, expected) do
      :ok ->
        :ok
...
defmodule MatchedIFBenchGepa014Imp.Runner do
  alias Imp.Optimizer.{Artifact, GEPA, MIPROv2, Report}
  alias MatchedIFBenchGepa014Imp.{ObservedLM, Observer}

  @manifest Path.expand("contract.json", __DIR__)
  @output Path.expand(
            System.get_env(
              "IMP_MATCHED_IFBENCH_GEPA014_OUTPUT",
              "../../tmp/matched_gepa_mipro_ifbench_gepa014/imp-result.json"
            ),
            __DIR__
          )
  ...
  def run do
    ...
    try do
      ...
      {sealed, held_out} =
        MatchedGepaMiproIFBenchGepa014.TwoPhase.seal_then_load_held_out!(
          work_items,
  ...
  defp wait_for_peer_selection!(manifest) do
    peer =
      System.get_env(
        ...
        Path.expand(
          "../../tmp/matched_gepa_mipro_ifbench_gepa014/upstream-result.json.selection-sealed.json",
          __DIR__
        )
      )
  ...
  defp evaluate(program, rows, observer, seed, arm, phase, expected_model) do
    ...
    Enum.map(rows, fn row ->
      ...
      evidences =
        Enum.map(
          responses,
          &MatchedGepaMiproIFBenchGepa014.ResponseEvidence.from_result!(&1.result)
        )
  ...
  defp validate_response_ledger!(before, after_snapshot, manifest) do
    ...
    Enum.each(responses, fn entry ->
      response = MatchedGepaMiproIFBenchGepa014.ResponseEvidence.from_result!(entry.result)
  ...
  defp validate_transport_evidence!(evidence, expected, context) do
    ...
    unless evidence.model in [configured, expected["imp"]] and
             ...
             MatchedGepaMiproIFBenchGepa014.ResponseEvidence.costs_reconcile?(
               evidence.gateway_reported_cost,
  ...
  defp source_commits!(manifest, clean?, expected \\ require_expected_launch_commit!()) do
    ...
    commits =
      if clean?,
        do: MatchedGepaMiproIFBenchGepa014.SourceIdentity.capture_clean!(root, pinned),
        else: MatchedGepaMiproIFBenchGepa014.SourceIdentity.current(root, pinned)
  ...
  defp require_expected_launch_commit! do
    case System.get_env("MATCHED_IFBENCH_GEPA014_EXPECTED_COMMIT") do
      value when is_binary(value) and byte_size(value) == 40 -> value
      _ -> raise "MATCHED_IFBENCH_GEPA014_EXPECTED_COMMIT is required as a full commit"
    end
  end
  ...
  defp rescue_accounting(snapshot) do
    MatchedIFBenchGepa014Imp.StopAccounting.normalize(
  ...
  defp empty_rescue_accounting, do: MatchedIFBenchGepa014Imp.StopAccounting.empty()
