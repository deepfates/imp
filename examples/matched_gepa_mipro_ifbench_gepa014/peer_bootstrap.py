...
MODE_SUBSTITUTION_KEYS = frozenset(
    {
        "OPENROUTER_API_KEY",
        "MATCHED_IFBENCH_GEPA014_API_BASE_URL",
        "MATCHED_IFBENCH_GEPA014_CATALOG_BASE_URL",
        "MATCHED_IFBENCH_GEPA014_HEALTH_URL",
        "MATCHED_IFBENCH_GEPA014_TLS_CA_CERT",
        "SSL_CERT_FILE",
    }
)
...
class PeerMode:
    ...
    def substitutions(self) -> dict[str, str]:
        return {
            ...
            "MATCHED_IFBENCH_GEPA014_API_BASE_URL": self.api_base_url,
            "MATCHED_IFBENCH_GEPA014_CATALOG_BASE_URL": self.catalog_base_url,
            "MATCHED_IFBENCH_GEPA014_HEALTH_URL": self.health_url,
            "MATCHED_IFBENCH_GEPA014_TLS_CA_CERT": self.tls_ca_cert,
...
def canonical_spec(
    ...
    common = {
        ...
        "IMP_MATCHED_IFBENCH_GEPA014_OUTPUT": str(tmp / "imp-result.json"),
        "IMP_MATCHED_IFBENCH_GEPA014_UPSTREAM_SELECTION": str(
        ...
        "UPSTREAM_MATCHED_IFBENCH_GEPA014_OUTPUT": str(tmp / "upstream-result.json"),
    }
    return {
...
def build_environment(
    ...
    env["MATCHED_IFBENCH_GEPA014_BOOTSTRAP_SPEC"] = canonical_json(spec)
    env["MATCHED_IFBENCH_GEPA014_BOOTSTRAP_DIGEST"] = canonical_digest(spec)
    return env
...
def require_runtime_environment(
    ...
    if environment.get("MATCHED_IFBENCH_GEPA014_BOOTSTRAP_DIGEST") != expected_digest:
        raise RuntimeError("peer bootstrap digest mismatch")
    if environment.get("MATCHED_IFBENCH_GEPA014_BOOTSTRAP_SPEC") != canonical_json(
        expected_spec
    ):
