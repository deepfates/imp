...
class PairedCoordinatorTest(unittest.TestCase):
    ...
    def test_failure_requires_both_cost_preserving_stop_artifacts(self) -> None:
        ...
        binding = {
            ...
            "failure_cardinality_gate_result_sha256": manifest[
    ...
    def test_upstream_rescue_distinguishes_failed_and_in_flight_transports(
        self,
    ) -> None:
        manifest = json.loads((HERE / "contract.json").read_text())
        accounting = {
        ...
        counts = paired.validate_rescue_accounting(
            "upstream",
            {"rescue_accounting": accounting, "calls": [{}], "response_ledger": []},
            manifest,
        )
        self.assertEqual(counts["total_logical"], 1)

        failed = json.loads(json.dumps(accounting))
        failed["ledger"].update({"failed": 1, "in_flight": 0})
        paired.validate_rescue_accounting(
            "upstream",
            {"rescue_accounting": failed, "calls": [{}], "response_ledger": []},
            manifest,
        )

        fabricated = json.loads(json.dumps(accounting))
        fabricated["ledger"].update({"completed": 1, "in_flight": 0})
        with self.assertRaisesRegex(RuntimeError, "response ledger was not retained"):
            paired.validate_rescue_accounting(
                "upstream",
                {"rescue_accounting": fabricated, "calls": [{}], "response_ledger": []},
                manifest,
            )
    ...
    def test_imp_rescue_certifies_transmitted_but_incomplete_calls(self) -> None:
        ...
        fabricated = json.loads(json.dumps(accounting))
        fabricated["ledger"].update({"completed": 2, "in_flight": 0})
        with self.assertRaisesRegex(RuntimeError, "response ledger was not retained"):
            paired.validate_rescue_accounting(
                "imp",
                {
    ...
    def test_barrier_validation_uses_content_bound_delegated_implementation(
        ...
        self.assertEqual(
            paired.sha256_file(
                (HERE / manifest["execution_base"]["runner_path"]).resolve()
            ),
            manifest["execution_base"]["runner_sha256"],
        )
    ...
    def test_upstream_direct_entry_requires_launch_commit_before_argument_parse(
        ...
        env = dict(os.environ)
        env.pop("MATCHED_IFBENCH_GEPA014_EXPECTED_COMMIT", None)
        ...
        self.assertIn(
            "MATCHED_IFBENCH_GEPA014_EXPECTED_COMMIT is required", completed.stdout
        )
    ...
    def test_wrong_launch_commit_refuses_before_optional_import(self) -> None:
        env = dict(os.environ)
        env["MATCHED_IFBENCH_GEPA014_EXPECTED_COMMIT"] = "0" * 40
        env.pop("OPENROUTER_API_KEY", None)
        completed = subprocess.run(
            [sys.executable, str(HERE / "run_upstream.py")],
            cwd=HERE.parents[1],
        ...
        self.assertNotEqual(completed.returncode, 0)
        self.assertIn("LaunchAdmissionError", completed.stdout)
        self.assertIn("successor launch commit drift", completed.stdout)
        self.assertNotIn("ModuleNotFoundError", completed.stdout)
    ...
    def test_upstream_stop_writer_binds_context_and_normalizes_ledgers(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as root, mock.patch.dict(
            os.environ, {"MATCHED_IFBENCH_GEPA014_EXPECTED_COMMIT": launch_commit}
        ):
        ...
        self.assertEqual(
            stopped["failure_cardinality_gate_result_sha256"],
            upstream.manifest_contract["failure_cardinality_compat"]["result_sha256"],
        )
        self.assertEqual(
