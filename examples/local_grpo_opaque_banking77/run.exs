defmodule LocalGRPOOpaqueBanking77.Atomic do
...
defmodule LocalGRPOOpaqueBanking77.Reward do
  def exact_route(expected, prediction, %{}) do
    if Imp.get(prediction, :route) == Imp.get(expected, :route), do: 1.0, else: 0.0
  end
end

defmodule LocalGRPOOpaqueBanking77.Definition do
  @instruction """
  Classify the customer request into exactly one opaque route code.
  The only valid route codes are R17, R42, R68, and R93.
  """

  def instruction, do: @instruction
  def train_steps, do: 38
  def train_width, do: 4

  def train_kwargs,
    do: [learning_rate: 1.0e-6, beta: 0.0, loss_type: :dapo, scale_rewards: :group]
end

defmodule LocalGRPOOpaqueBanking77.Runner do
  alias Imp.Clients.{TrainingJob, TRLArtifact, TRLDeployment, TRLLM, TRLProtocol, TRLTrainer}
  alias Imp.Optimizer.GRPO.Callback
  alias LocalGRPOOpaqueBanking77.Atomic
  alias LocalGRPOOpaqueBanking77.Definition

  @data_sha256 "1703f59bf336df8dc35590275531b67bb6ee43a5d0c96eb44696c219af5cfc18"
  @train_sha256 "sha256:16d37a946696995fe343b2773e546f54c2d0a1a97adfc0fbce1782e79e281f34"
  @selection_sha256 "sha256:5b34859cb5e6588dcc53360c68e76875c0fba2191b55a7169b4781acef97c6c1"
  @test_sha256 "sha256:5d70b2ff26f9c30862175e63bc1cb4742f509f49c7571e205f1faf06d5d10fe1"
  @contract_sha256 "848762a028f3340265e69ff8e09228f13f25aeba1c65ae601868022b41daafe3"
  @model "Qwen/Qwen2.5-0.5B-Instruct@7ae557604adf67be50417f59c2c2f167def9a775"
  @seed 20_260_725
  @treatment_id "model-generated-banking77-opaque-routes-two-padded-schedules-v1"
  @routes ["R17", "R42", "R68", "R93"]
  @train_ids ~w(
    banking77-train-2511 banking77-train-2512 banking77-train-2513 banking77-train-2514
    banking77-train-2515 banking77-train-2516 banking77-train-2517 banking77-train-2518
    banking77-train-2519 banking77-train-2520 banking77-train-2521 banking77-train-2522
    banking77-train-2523 banking77-train-2524 banking77-train-2525 banking77-train-2526
    banking77-train-2527 banking77-train-2528
    banking77-train-3482 banking77-train-3483 banking77-train-3484 banking77-train-3485
    banking77-train-3486 banking77-train-3487 banking77-train-3488 banking77-train-3489
    banking77-train-3490 banking77-train-3491 banking77-train-3492 banking77-train-3493
    banking77-train-3494 banking77-train-3495 banking77-train-3496 banking77-train-3497
    banking77-train-3498 banking77-train-3499
    banking77-train-5092 banking77-train-5093 banking77-train-5094 banking77-train-5095
    banking77-train-5096 banking77-train-5097 banking77-train-5098 banking77-train-5099
    banking77-train-5100 banking77-train-5101 banking77-train-5102 banking77-train-5103
    banking77-train-5104 banking77-train-5105 banking77-train-5106 banking77-train-5107
    banking77-train-5108 banking77-train-5109
    banking77-train-6722 banking77-train-6723 banking77-train-6724 banking77-train-6725
    banking77-train-6726 banking77-train-6727 banking77-train-6728 banking77-train-6729
    banking77-train-6730 banking77-train-6731 banking77-train-6732 banking77-train-6733
    banking77-train-6734 banking77-train-6735 banking77-train-6736 banking77-train-6737
    banking77-train-6738 banking77-train-6739
  )
  @selection_ids ~w(
    banking77-train-2529 banking77-train-2530 banking77-train-3500 banking77-train-3501
    banking77-train-5110 banking77-train-5111 banking77-train-6740 banking77-train-6741
  )
  ...
  defp resume_preflight do
    ...
    IO.puts("opaque-route GRPO resume preflight complete")
  end
  ...
  defp parent do
    paths = paths!()
    resume? = System.get_env("IMP_GRPO_OPAQUE_RESUME") == "1"
  ...
  defp run_parent(paths, resume?) do
    ...
    try do
      ...
      summary = %{
        ...
        source_schedule: source_schedule!(step_artifacts, rows.train),
  ...
  defp train!(paths, rows, trainer) do
    ...
    optimizer =
      Imp.Optimizer.GRPO.new(
        Callback.reward(LocalGRPOOpaqueBanking77.Reward, :exact_route,
          id: "banking77-opaque-exact-route-v1",
          config: %{}
        ),
        trainer: trainer,
    ...
    source_schedule!(step_artifacts, rows.train)
  ...
  defp fresh do
    ...
    try do
      ...
      Atomic.write!(System.fetch_env!("IMP_GRPO_OPAQUE_FRESH_OUTPUT"), %{
        status: "complete",
  ...
  defp verify_retained_preflight!(paths, current) do
    ...
    unless retained_runner == current_runner,
  ...
  # DSPy/mmGRPO pads every 72-row schedule by one full width-four batch even
  # when the source length is divisible. Thirty-eight steps therefore bind two
  # complete 76-slot schedules: all 72 rows occur at least twice, exactly eight
  # least-frequent rows occur a third time, and no row may be absent or occur
  # more often. This is pinned scheduler behavior, not a claim of two unpadded
  # epochs.
  defp source_schedule!(step_artifacts, rows) do
  ...
  defp fresh_process(paths, output) do
    env = [
      {"MIX_ENV", "dev"},
      {"IMP_GRPO_OPAQUE_FRESH", "1"},
      {"IMP_GRPO_OPAQUE_OUTPUT", paths.output},
      {"IMP_GRPO_OPAQUE_FRESH_OUTPUT", output},
    ...
    System.cmd(
      "mix",
      ["run", "--no-compile", "--no-deps-check", "examples/local_grpo_opaque_banking77/run.exs"],
      cd: paths.repo,
      env: env,
  ...
  defp require_new_output!(path) do
    case File.ls(path) do
      {:error, :enoent} -> :ok
      {:ok, []} -> :ok
      {:ok, _entries} -> raise("IMP_GRPO_OPAQUE_OUTPUT must be a new empty directory")
      {:error, reason} -> raise("cannot inspect IMP_GRPO_OPAQUE_OUTPUT: #{inspect(reason)}")
    end
  end
  ...
  defp require_resume_output!(paths) do
    ...
    unless Enum.all?(Enum.take(required, 2), &File.regular?/1) and
           ...
           do:
             raise(
               "IMP_GRPO_OPAQUE_RESUME requires retained preflight/base plus a checkpoint or completed training stages"
             )
...
unless System.get_env("IMP_GRPO_OPAQUE_DEFINE_ONLY") == "1",
  do: LocalGRPOOpaqueBanking77.Runner.run()
