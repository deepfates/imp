...
| --- | --- | --- | --- | --- | --- |
| V2-OA-001 | P0 | `DSPy.Optimize.Anything` optimizes arbitrary text artifacts using a declarative artifact spec, train/val examples, evaluator feedback, diagnostics, and reproducible search configuration. | PARTIAL | `test/optimize_anything_test.exs` covers improvement, baseline retention, deterministic lineage, artifact kinds, and report save/load. | Add evaluator error handling, richer artifact specs, benchmark integration, docs examples, and then mark PROVEN only when all criteria are covered. |
