...
| --- | --- | --- | --- | --- | --- |
| V2-OA-001 | P0 | `DSPy.Optimize.Anything` optimizes arbitrary text artifacts using a declarative artifact spec, train/val examples, evaluator feedback, diagnostics, and reproducible search configuration. | UNPROVEN | None yet. | Public API supports prompt/code/config/string artifacts; evaluator returns score plus diagnostics; optimizer returns best artifact, candidate history, lineage, and reproducible report; tests prove improvement, non-regression baseline retention, deterministic seeds, serialization, and error handling. |
