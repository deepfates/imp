...
| V2-BENCH-001 | P0 | Benchmark fixtures compare Ax-style signatures and GEPA optimize-anything tasks with reproducible scores and production-gate reporting. | PROVEN | `test/v2_benchmark_test.exs` and `DSPy.V2.Benchmarks` cover structured extraction, agent/tool execution, GEPA prompt optimization, and arbitrary config optimization with deterministic thresholds. | Add larger optional benchmark suites outside the required production gate. |
| V2-DOC-001 | P1 | V2 docs explain APIs, completion evidence, examples, migration notes from V1, and honest remaining non-goals. | PROVEN | `V2.md`, `README.md`, and `V2_ROADMAP.md` include runnable examples, gates, migration notes, and non-goals. | Keep examples synchronized with tests as APIs evolve. |
