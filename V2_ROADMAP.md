...
| --- | --- | --- | --- | --- | --- |
| V2-OA-001 | P0 | `DSEx.Optimize.Anything` optimizes arbitrary text artifacts using a declarative artifact spec, train/val examples, evaluator feedback, diagnostics, and reproducible search configuration. | PROVEN | `test/optimize_anything_test.exs` covers improvement, baseline retention, deterministic lineage, prompt/code/config/text artifact kinds, named parameters, diagnostics, evaluator error handling, and report save/load. | Keep API stable while integrating V2 benchmarks and docs. |
| V2-GEPA-001 | P0 | GEPA supports Pareto-aware reflective evolution with Actionable Side Information (ASI), mutation, candidate lineage, and system-aware merge. | PROVEN | `test/optimize_gepa_test.exs` covers Pareto frontier selection, ASI-driven mutation, reflection-LM proposer, lineage, system-aware merge, single/multi-task behavior, and dev-example held-out selection. | Continue comparing against upstream GEPA examples as they stabilize, but the V2 completion criteria are covered by executable tests. |
| V2-AGENT-001 | P0 | Agent runtime affordances cover typed agents, flows, runtime sessions, memory/context fields, tool execution, and child/specialist agents. | PROVEN | `test/agent_runtime_test.exs` covers runtime sessions, memory/context refs, arity-3 self-aware handlers without process dictionary self-reference, tool policies, failures, child agents, final-output streaming, and incremental trace-event streaming. | Add larger multi-agent workflow examples over time, but the V2 completion criteria are covered by executable tests. |
| V2-MCP-001 | P1 | MCP-style tool discovery/import exposes external tool catalogs as typed `DSEx.Tool` values without hard-coding tool schemas. | PROVEN | `test/mcp_import_test.exs` covers in-process catalog import, transport-backed HTTP discovery, remote tool execution, atom/string schema keys, JSON-schema property validation, duplicate-name rejection, malformed schema errors, and string-key validation without atomizing external keys. | Add live real-server contract fixtures when a stable MCP test server is available. |
...
| V2-DOC-001 | P1 | V2 docs explain APIs, completion evidence, examples, migration notes from V1, and honest remaining non-goals. | PROVEN | `V2.md`, `README.md`, and `V2_ROADMAP.md` describe V2 completion criteria, state that gates fail while partial rows remain, and avoid presenting deterministic benchmark fixtures as public leaderboard claims. | Keep docs synchronized with audit rows as implementation status changes. |
