defmodule Imp.Clients.MLXLMTrainer do
  @moduledoc """
  ...
  def finetune(%__MODULE__{} = trainer, _deployment_lm, examples, opts) do
    ...
    error -> {:error, {:mlx_lm_training_failed, Imp.Redaction.redact(Exception.message(error))}}
  catch
    kind, reason ->
      {:error, {:mlx_lm_training_failed, Imp.Redaction.redact(inspect({kind, reason}))}}
  end
  ...
  defp call_config(trainer, opts) do
    ...
    if match?(%Imp.Signature{}, signature) and is_atom(adapter) do
      {:ok,
       %{
         signature: signature,
         adapter: adapter,
  ...
  defp command_summary(result) do
    %{
      ...
      "output" => result |> Map.get(:output, "") |> Imp.Redaction.redact()
    }
  end
  ...
  defp default_root, do: Path.join([System.user_home!(), ".cache", "imp", "mlx_lm", "sft"])
  ...
  defp redact_term(value), do: Imp.Redaction.redact(value)
