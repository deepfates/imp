defmodule Imp.Training.ChatDataset do
  @moduledoc """
  Deterministically materializes Imp examples as MLX-LM chat JSONL.

  Prompts and assistant completions are rendered by the configured Imp adapter.
  ...
  @spec build([Imp.Example.t()], Imp.Signature.t(), module(), keyword()) ::
          {:ok, dataset()} | {:error, term()}
  def build(examples, signature, adapter, opts \\ [])

  def build(examples, %Imp.Signature{} = signature, adapter, opts)
    ...
    error -> {:error, {:chat_dataset_failed, Imp.Redaction.redact(Exception.message(error))}}
  end
  ...
  defp encode_examples(examples, signature, adapter, stratify_by) do
    ...
    |> Enum.reduce_while({:ok, []}, fn
      {%Imp.Example{} = example, index}, {:ok, acc} ->
  ...
  defp encode_example(example, signature, adapter) do
    ...
    error -> {:error, {:adapter_render_failed, Imp.Redaction.redact(Exception.message(error))}}
  end
  ...
  defp normalize_message(message) when is_map(message) do
    ...
    if role in [:system, :user, :assistant, "system", "user", "assistant"] and is_binary(content) do
      ...
      {:error, {:unsupported_chat_message, Imp.Redaction.redact(message)}}
    end
  end
  ...
  defp stratum(example, fields) do
    fields
    |> Enum.map(fn field -> {to_string(field), Imp.Example.get(example, field)} end)
    |> Map.new()
  end
