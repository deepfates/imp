defmodule Imp.MCP.CallFailure do
  ...
  so a host decides what to do without matching ExMCP's terms:
    ...
    * `:not_sent` — the request never left: the connection could not be
      ...
      the client process was already gone, or every connection to the server
      stayed busy until the call's timeout (`:no_idle_connection`).
  ...
  call a client of its own (`pool_size:` in `Imp.MCP.Connections`), so a call
  waits for a client rather than inside one, and a call that never got one is
  `:not_sent`.
