defmodule Imp.Predict.Refine do
  @moduledoc """
  ...
  def new(program, metric, opts \\ []) do
    opts = Imp.Options.validate!(opts, @option_schema, "Imp.Predict.Refine.new/3")
    Imp.FunctionContract.validate!(metric, 2, "Imp.Predict.Refine.new/3", "metric")
  ...
  defp attach_history(prediction, outcomes) do
    ...
    Imp.Prediction.put(prediction, :refine_history, history)
  end
  ...
  defp feedback_lm(program) do
    ...
    predictor_lm || Imp.Settings.get().lm
  rescue
    _error -> nil
  end
  ...
  defp automatic_feedback(refine, lm, inputs, outcomes, metric_result) do
    ...
    feedback_inputs = %{
      ...
      program_inputs: Imp.Redaction.redact(Map.new(inputs)),
  ...
  defp feedback_signature do
    Imp.Signature.new(%{
      inputs: [
        ...
        %{name: :reward_code, desc: "The executable reward contract available to Imp."},
  ...
  defp predictor_definition(%{name: name, predictor: predictor}) do
    %{
      ...
      config: Imp.Redaction.redact(Map.get(predictor, :config, []))
    }
  end

  defp signature_definition(%Imp.Signature{} = signature), do: Imp.Signature.dump(signature)
  ...
  defp per_predictor_trajectory(outcomes, predictor_names) do
    trajectory =
      Enum.map(outcomes, fn %{attempt: attempt, prediction: prediction} ->
        %{
          attempt: attempt,
          output: Imp.Redaction.redact(Imp.Prediction.to_map(prediction)),
  ...
  defp outputs(outcomes) do
    Enum.map(outcomes, fn %{attempt: attempt, prediction: prediction} ->
      %{attempt: attempt, output: Imp.Redaction.redact(Imp.Prediction.to_map(prediction))}
    end)
  end
  ...
  defp parse_advice(prediction, predictor_names) do
    advice = Imp.Prediction.get(prediction, :advice, %{})
  ...
  defp advice_instructions(signature, name, advice) do
    (signature.instructions || "") <>
      "\n\nRepair advice for #{name}: #{advice |> inspect() |> Imp.Redaction.redact()}"
  end
  ...
  defp safe_reason(reason) when is_binary(reason), do: Imp.Redaction.redact(reason)
  ...
  defp safe_reason(reason) when is_map(reason), do: Imp.Redaction.redact(reason)
  defp safe_reason(reason), do: Imp.Redaction.redact(Attempt.error_message(reason))
end
