defmodule Imp.Predict.Assertions do
  @moduledoc """
  ...
  def new(program, assertions, opts \\ []) do
    opts = Imp.Options.validate!(opts, @option_schema, "Imp.Predict.Assertions.new/3")
    assertions = assertions |> List.wrap() |> Enum.map(&Imp.Assertion.normalize!/1)

    if assertions == [] do
      raise ArgumentError, "Imp.Predict.Assertions.new/3 expects at least one assertion"
    end
  ...
  def call(%__MODULE__{} = wrapper, inputs) do
    ...
    |> Enum.reduce_while(%{best: nil, history: [], last_error: nil}, fn attempt, state ->
      ...
      case Imp.Module.call(wrapper.program, attempt_inputs) do
        {:ok, prediction} ->
  ...
  defp evaluate_assertion(%Imp.Assertion{} = assertion, inputs, prediction) do
    case call_predicate(assertion.predicate, inputs, prediction) do
      true ->
        %{name: assertion.name, passed?: true, message: assertion.message}

      %Imp.Metrics.Result{} = result ->
        %{
          name: assertion.name,
          passed?: Imp.Metrics.pass?(result),
  ...
  defp attach_assertion_metadata(prediction, evaluation, history) do
    prediction
    |> Imp.Prediction.put(:assertion_score, evaluation.score)
    |> Imp.Prediction.put(:assertion_failures, evaluation.failures)
    |> Imp.Prediction.put(:assertion_history, history)
  end
end
