defmodule Imp.Predict.Predict do
  ...
  @doc false
  # The options of a wrapper's own validated options that are `new/2`'s.
  def take_options(opts), do: Keyword.take(opts, Keyword.keys(@option_schema))
  ...
  def validate_options!(opts, schema, context) when is_list(opts) do
  ...
  def validate_options!(opts, schema, context), do: Imp.Options.validate!(opts, schema, context)

  @doc """
  Builds a prediction program.

  Pass `:lm` and `:adapter` for a self-contained program, or omit them to resolve
  from process/global Imp settings at call time. An unknown option raises
  `ArgumentError`.
  ...
  defp validate_opts!(opts) do
    {opts, validate_options!(opts, @option_schema, "Imp.Predict.Predict.new/2")}
  end
