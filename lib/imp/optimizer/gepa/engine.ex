defmodule Imp.Optimizer.GEPA.Engine do
  ...
  def dump_state(%State{} = state) do
    ...
    checkpoint = %{
      ...
      "rejected" => Imp.Optimizer.Report.json_safe(state.rejected),
      "history" => Imp.Optimizer.Report.json_safe(state.history),
      ...
      "merge_attempts" => Imp.Optimizer.Report.json_safe(state.merge_attempts),
      ...
      "stop_reason" => Imp.Optimizer.Report.json_safe(state.stop_reason)
    }
  ...
  defp dump_entry(%Entry{} = entry) do
    %{
      "id" => entry.id,
      "candidate" => Imp.Optimizer.Report.json_safe(entry.candidate),
  ...
  defp dump_result(%Result{} = result) do
    %{
      ...
      "scores" => result.scores,
      "objective_scores" => Imp.Optimizer.Report.json_safe(result.objective_scores),
      "trajectories" =>
      ...
      "side_information" => Imp.Optimizer.Report.json_safe(result.side_information),
      "metadata" => Imp.Optimizer.Report.json_safe(result.metadata)
    }
  end
  ...
  defp dump_cache(cache) do
    Enum.map(cache, fn {{candidate_digest, example_digest}, %EvaluationCache.Entry{} = entry} ->
      %{
        ...
        "score" => entry.score,
        "objective_scores" => Imp.Optimizer.Report.json_safe(entry.objective_scores)
      }
    end)
  end
  ...
  defp dump_runtime_term(%Imp.Prediction{} = prediction) do
    %{
      ...
      "fields" => Imp.Optimizer.Report.json_safe(prediction.fields),
      "completions" => Imp.Optimizer.Report.json_safe(prediction.completions),
      "score" => prediction.score,
      "metadata" => Imp.Optimizer.Report.json_safe(prediction.metadata)
    }
  end
  ...
  defp dump_runtime_term(term), do: Imp.Optimizer.Report.json_safe(term)
  ...
  defp restore(value), do: Imp.Optimizer.Report.restore_json_safe(value)
