defmodule Imp.Optimizer.BootstrapFewShot do
  ...
  Compilation also attaches a `Imp.Optimizer.Report` so you can inspect which
  ...
  def compile(%__MODULE__{} = optimizer, program, trainset) do
    {demos_by_name, selected_count, candidates, errors} =
      case materialize_trainset(trainset) do
        {:ok, examples} ->
          trajectories =
            Imp.Optimizer.TrajectoryRunner.run(program, examples, optimizer.metric,
              runtime: :evaluation
            )
          ...
          demos_by_name = Imp.Optimizer.DemoCandidates.extract_bootstrapped(selected, names)
    ...
    report =
      Imp.Optimizer.Report.new(%{
        ...
        metadata: %{
          ...
          trajectory_substrate: Imp.Optimizer.Trajectory,
  ...
  defp put_demos(program, demos_by_name) do
    Enum.reduce(demos_by_name, program, fn {name, demos}, compiled ->
      Imp.ProgramParameters.put_demos(compiled, name, demos)
    end)
  end
      ...
      nil ->
        Enum.reduce(Imp.ProgramParameters.predictors(program), program, fn %{name: name}, acc ->
          Imp.ProgramParameters.update_predictor(acc, name, fn predictor ->
            Imp.Optimizer.Report.attach(predictor, report)
          end)
        end)
