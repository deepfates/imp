defmodule Imp.Optimizer.BetterTogether do
  ...
  rebound `Imp.Optimizer.TrainingResult`. Creating an asynchronous training job
  is reported as an incomplete step and stops the sequence without pretending
  that weight optimization occurred.
  """

  alias Imp.Optimizer.{BootstrapFinetune, Report, Sampling}
  ...
  def new(metric, optimizers \\ %{}) do
    Imp.FunctionContract.validate!(metric, 2, "Imp.Optimizer.BetterTogether.new/2", "metric")
  ...
  defp compile_step(optimizer, program, trainset, valset) do
    ...
    case Imp.Optimizer.run_with_datasets(optimizer, program, datasets, [:program, :training]) do
      {:ok, %{kind: :program}, compiled} ->
        {:ok, compiled, %{optimizer: optimizer.__struct__, kind: :program}}

      {:ok, %{kind: :training}, %Imp.Optimizer.TrainingResult{status: :completed} = result} ->
        {:ok, result.program,
         %{optimizer: optimizer.__struct__, kind: :training, training_status: result.status}}

      {:ok, %{kind: :training}, %Imp.Optimizer.TrainingResult{status: status}} ->
