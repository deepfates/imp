defmodule Imp.Optimizer.SIMBA.Checkpoint do
  @moduledoc false

  alias Imp.Optimizer.{Report, Sampling, SearchPolicy}
  alias Imp.Optimizer.SIMBA.Population

  @type_name "imp_simba_run"
  @schema_version 1
  ...
  defp dump_program(program) do
    %{
      "predictors" =>
        Enum.map(Imp.ProgramParameters.predictors(program), fn %{
                                                                 name: name,
                                                                 predictor: predictor
                                                               } ->
          %{
  ...
  defp load_program!(%{"predictors" => snapshots}, runtime_program) when is_list(snapshots) do
    runtime_predictors = Imp.ProgramParameters.predictors(runtime_program)
    ...
    Enum.reduce(decoded, runtime_program, fn snapshot, program ->
      ...
      |> Imp.ProgramParameters.put_demos(snapshot.name, demos)
    end)
  end
