defmodule Imp.ExternalCommand.Handle do
  @moduledoc "A supervised external process group managed by `Imp.ExternalCommand`."
...
defmodule Imp.ExternalCommand.Lifecycle do
  @moduledoc false
  ...
  @spec start(String.t(), [String.t()], keyword()) ::
          {:ok, Imp.ExternalCommand.Handle.t()} | {:error, term()}
  ...
  def stop(%Imp.ExternalCommand.Handle{} = handle, timeout \\ 10_000) do
  ...
  defp start_owner(executable, argv, opts, require_os_pid?) do
    with :ok <- validate_command(executable, argv),
      ...
      case Task.Supervisor.start_child(Imp.UnlinkedTaskSupervisor, fn ->
             port_owner(
               caller,
               ref,
               executable_path,
  ...
  defp await_started(ref, owner) do
    ...
    receive do
      {^ref, {:started, os_pid}} ->
        Process.demonitor(monitor_ref, [:flush])
        {:ok, %Imp.ExternalCommand.Handle{owner: owner, os_pid: os_pid, ref: ref}}
  ...
  defp encode_env(env) do
    ...
    [{~c"IMP_BEAM_PORT_OWNER", ~c"1"} | encoded]
  end
...
defmodule Imp.ExternalCommand do
  @moduledoc """
