defmodule Imp.BenchmarkTruth.IFBenchFeedback do
  @moduledoc false

  alias Imp.Optimizer.GEPA.ComponentFeedback

  @type metric ::
          (Imp.Example.t(), Imp.Prediction.t() -> term())
          | (Imp.Example.t(), Imp.Prediction.t(), [map()] -> term())
  ...
  defp feedback_callback(metric, output_key) do
    fn %ComponentFeedback{} = context ->
      prediction =
        Imp.Prediction.new(response: fetch_output!(context.predictor_output, output_key))
  ...
  defp feedback_text!(result, output_key) do
    case Imp.Metrics.feedback(result) do
