defmodule Imp.BenchmarkTruth.Composition do
  @moduledoc false
  ...
  defmodule AnswerProgram do
    ...
    def call(%__MODULE__{} = program, _inputs),
      do: {:ok, Imp.Prediction.new(answer: program.answer)}
  end
  ...
  defmodule QuestionProgram do
    ...
    def call(%__MODULE__{}, inputs) do
      ...
      cond do
        ...
        question =~ "France" -> {:ok, Imp.Prediction.new(answer: "Paris")}
        question =~ "Germany" -> {:ok, Imp.Prediction.new(answer: "Berlin")}
        true -> {:ok, Imp.Prediction.new(answer: "unknown")}
      end
    end
  end
  ...
  defp best_of_n([example | _examples]) do
    ...
    {:ok, _preflight} = Imp.Module.call(program, example)
    best = Imp.Predict.BestOfN.new(program, metric, n: 3)

    {:ok, prediction} = Imp.Predict.BestOfN.call(best, example)
  ...
  defp refine([example | _examples]) do
    ...
    %{
      ...
      "score" => score,
      "passed" => score == 1.0,
      "history_length" => prediction |> Imp.Prediction.get(:refine_history, []) |> length(),
      "selected_prediction" => prediction_summary(prediction)
    }
  end
  ...
  defp ensemble([example | _examples]) do
    reducer = fn predictions ->
      predictions
      |> Enum.map(&Imp.Prediction.get(&1, :answer))
      |> Enum.frequencies()
      |> Enum.max_by(fn {_answer, count} -> count end)
      |> elem(0)
      |> then(&Imp.Prediction.new(answer: &1))
    end
    ...
    {:ok, prediction} = Imp.Optimizer.Ensemble.Program.call(program, example)
    score = answer_score(prediction, example["answer"])
    ...
    {:ok, raw_prediction} = Imp.Optimizer.Ensemble.Program.call(raw, example)
    outputs = Imp.Prediction.get(raw_prediction, :outputs)
  ...
  defp knn(examples) do
    ...
    %{
      ...
      "score" => score,
      "passed" => score == 1.0,
      "selected_demos" => Enum.map(demos, &Imp.Example.to_map/1),
    ...
    successes =
      ...
      |> Enum.count(fn
        {{:ok, prediction}, %{question: question}} ->
          expected_parallel_answer(question) == Imp.Prediction.get(prediction, :answer)
  ...
  defp answer_score(prediction, answer) do
    if Imp.Metrics.normalize_text(Imp.Prediction.get(prediction, :answer)) ==
         Imp.Metrics.normalize_text(answer),
       do: 1.0,
       else: 0.0
  end
  ...
  defp example_from_row(row) do
    Imp.example(question: row["question"], answer: row["answer"])
  ...
  defp prediction_summary(%Imp.Prediction{} = prediction),
  ...
  defp json_safe(%Imp.Prediction{} = prediction), do: prediction_summary(prediction)
  defp json_safe(%Imp.Example{} = example), do: example |> Imp.Example.to_map() |> json_safe()
  ...
  defp example_map(%Imp.Example{} = example) do
    example
    |> Imp.Example.to_map()
