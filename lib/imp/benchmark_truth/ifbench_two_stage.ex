defmodule Imp.BenchmarkTruth.IFBenchTwoStage do
  @moduledoc false
    ...
    %__MODULE__{
      generate_response_module:
        Imp.chain_of_thought(
          Imp.signature("query -> response", @generate_instruction),
          opts
        ),
      ensure_correct_response_module:
        Imp.chain_of_thought(
          Imp.signature("query, response -> final_response", @ensure_instruction),
          opts
        )
    }
  end
  ...
  def call(%__MODULE__{} = program, inputs) when is_map(inputs) or is_list(inputs) do
    with {:ok, query} <- fetch_prompt(inputs),
         {:ok, response} <-
           Imp.Module.call(program.generate_response_module, %{query: query}),
         {:ok, final_response} <-
           Imp.Module.call(program.ensure_correct_response_module, %{
             query: query,
             response: Imp.Prediction.fetch!(response, :response)
           }) do
      {:ok,
       Imp.Prediction.new(
         [response: Imp.Prediction.fetch!(final_response, :final_response)],
