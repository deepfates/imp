defmodule Imp.BenchmarkTruth.RLMProtocol do
  @moduledoc false
  ...
  @paper_runtimes ~w(imp standalone_rlm)
  ...
  defp model_roles_complete?(manifest) do
    Enum.all?(~w(root submodel compaction), fn role ->
      ...
      is_binary(model["logical"]) and is_binary(model["imp"]) and is_binary(model["dspy"]) and
