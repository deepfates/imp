defmodule Imp.IdentityAssessment do
  @moduledoc false
  ...
  defp invoke_assessor(profile, request, config) do
    lm =
      ReqLLMClient.new(profile.model,
        req_module: Imp.IdentityAssessment.ReqLLMTransport
      )
  ...
  defp parse_response(response) do
    with {:ok, output} <- Imp.LM.Result.output(response) do
      output
      |> unwrap_response()
      |> decode_response()
    end
  ...
  defp assessment_record(assessment, candidate, profile, assessed_at) do
    %{
      ...
      "assessor" => %{
        ...
        "transport" => "Imp.Clients.ReqLLM via ReqLLM"
      },
      "context" => %{},
      "scores" => assessment["scores"],
      "confidence" => assessment["confidence"],
...
defmodule Imp.IdentityAssessment.ReqLLMTransport do
  @moduledoc false
