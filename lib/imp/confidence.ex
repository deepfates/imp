defmodule Imp.Confidence do
  ...
  def evaluate(example, prediction, opts) do
    ...
    expected = example |> Imp.Example.get(expected_field) |> stringify()
    got = prediction_value(prediction, field)
    correct? = correct?(got, expected)

    metadata = if(match?(%Imp.Prediction{}, prediction), do: prediction.metadata, else: %{})

    case Imp.Capabilities.token_logprobs(metadata) do
      :ok ->
  ...
  defp evaluate_with_logprobs(
    ...
    case Imp.Logprobs.extract(content, logprobs, field, enum_values) do
      {:ok, extraction} ->
  ...
  defp additional_context(example, opts) do
    ...
    |> Enum.map(fn key -> {key, Imp.Example.get(example, key)} end)
    |> Map.new()
  end

  defp prediction_value(%Imp.Prediction{} = prediction, field),
    do: prediction |> Imp.Prediction.get(field) |> stringify()
