defmodule Imp.Optimize.Anything.Refiner do
  @moduledoc false
  ...
  defp parse_proposal(%Imp.Prediction{} = prediction, current_params) do
    prediction
  ...
  defp normalize_evaluation!({score, asi}) when is_number(score) and is_map(asi),
    do: %{score: score, output: nil, asi: Imp.Redaction.redact(asi)}

  defp normalize_evaluation!({score, output, asi}) when is_number(score) and is_map(asi),
    do: %{score: score, output: Imp.Redaction.redact(output), asi: Imp.Redaction.redact(asi)}
  ...
  defp successful_attempt(iteration, candidate, evaluation) do
    %{
      "iteration" => iteration,
      "candidate" => Imp.Redaction.redact(candidate),
  ...
  defp maybe_put_raw_output(attempt, raw_output) do
    Map.put(
      attempt,
      "raw_output",
      raw_output |> to_string() |> String.slice(0, 2_000) |> Imp.Redaction.redact()
    )
  end
  ...
  defp redact_error(reason) when is_binary(reason), do: Imp.Redaction.redact(reason)
