defmodule Dachshund do
  @moduledoc """
  Declarative self-improving language-model programs for Elixir.

  Dachshund programs are ordinary Elixir structs with explicit signatures,
  injectable model clients, measurable behavior, and optimizer-driven
  improvement loops.
  """

  alias Dachshund.{Example, Prediction, Settings, Signature}
  alias Dachshund.Predict.{ChainOfThought, Predict, ReAct}
  ...
  defdelegate majority(predictions, opts \\ []), to: Dachshund.Predict.Aggregation
  ...
  def openai(model, opts \\ []), do: Dachshund.Clients.OpenAI.new(model, opts)

  @doc "Creates a LiteLLM-compatible LM client."
  def litellm(model, opts \\ []), do: Dachshund.Clients.LiteLLM.new(model, opts)

  @doc "Creates a local OpenAI-compatible LM client."
  def local_lm(model, opts \\ []), do: Dachshund.Clients.Local.new(model, opts)

  @doc "Creates a Databricks OpenAI-compatible LM client."
  def databricks(model, opts \\ []), do: Dachshund.Clients.Databricks.new(model, opts)
end
