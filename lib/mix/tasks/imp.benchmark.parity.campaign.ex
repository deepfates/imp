defmodule Mix.Tasks.Imp.Benchmark.Parity.Campaign do
  @moduledoc """
  Run a chunked Imp-vs-DSPy parity campaign until coverage advances.

      mix imp.benchmark.parity.campaign \\
        --model "$IMP_PARITY_MODEL" \\
        --dspy-model "$IMP_PARITY_DSPY_MODEL" \\
  ...
  Set `IMP_PARITY_MODEL` to a provider model id you have verified in the
  current provider account, and set `IMP_PARITY_DSPY_MODEL` to the matching
  ...
  removes manual babysitting from long live campaigns. Imp chunks are scoped to
  ...
  defp run_aggregate!(model, out_dir, campaign_id, opts) do
    Mix.Task.reenable("imp.benchmark.parity.aggregate")

    Mix.Task.run(
      "imp.benchmark.parity.aggregate",
      [
        "--provider",
        "req_llm",
        "--model",
  ...
  defp run_chunk!(opts, model, campaign_id, chunk_plan, out_dir) do
    Mix.Task.reenable("imp.benchmark.parity")

    Mix.Task.run(
      "imp.benchmark.parity",
  ...
  defp runner_order_args(opts, offset) do
    order =
      case Keyword.get(opts, :runner_order, "alternate") do
        ...
        other ->
          Mix.raise(
            "--runner-order must be imp_first, dspy_first, or alternate, got: #{inspect(other)}"
          )
      end
  ...
  defp latest_chunk_has_runner_errors?(model, out_dir, campaign_id) do
    ...
    |> case do
      ...
      path ->
        ...
        |> Map.get("tasks", [])
        |> Enum.any?(fn task ->
          positive_error_count?(task["imp_errors"]) or positive_error_count?(task["dspy_errors"])
        end)
    end
  end
