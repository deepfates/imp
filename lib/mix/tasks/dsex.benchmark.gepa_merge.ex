defmodule Mix.Tasks.Dsex.Benchmark.GepaMerge do
  @moduledoc """
  Merge independently produced DSEx GEPA family chunks into one validated input.

      mix dsex.benchmark.gepa_merge chunk-a.json chunk-b.json --out benchmarks/results
  ...
  @shortdoc "Merge full-scope DSEx GEPA family chunks"
  ...
  def run(args) do
    ...
    out_path = Path.join(out_dir, "dsex-gepa-merged-#{timestamp_slug()}.json")
    out_path = DSEx.BenchmarkTruth.ArtifactFile.write_json!(out_path, artifact)
    Mix.shell().info("Merged DSEx GEPA rows: #{out_path}")
  end
  ...
  defp read_chunk!(path) do
    case path |> File.read!() |> Jason.decode!() do
      ...
      _ ->
        Mix.raise("#{path} is not a schema-valid DSEx GEPA campaign chunk")
    end
  end
