defmodule Mix.Tasks.Dsex.Benchmark.GepaReplication do
  ...
  defp gepa_artifact_rows!(artifact_dir, opts) do
    ...
    rows_from_upstream!({model, selected_results}, evidence, dsex_rows, campaign_id)
  end
  ...
  defp rows_from_upstream!({model, results}, evidence, dsex_rows, campaign_id) do
    grouped =
      unique_map!(
        results,
    ...
    Enum.map(required_family_programs(), fn {family, program, _budget} ->
      ...
      ensure_dsex_identity!(dsex, family, program, model, campaign_id)
      ...
      merge_comparator_evidence!(dsex, comparator_rows, campaign_id)
    end)
  end
  ...
  defp ensure_dsex_identity!(dsex, family, program, model, campaign_id) do
    expected = %{
      "family" => family,
      "program" => program,
      "model" => model,
    ...
    Enum.each(expected, fn {field, value} ->
      unless dsex[field] in [nil, value] do
        Mix.raise(
          "DSEx row #{family} has #{field}=#{inspect(dsex[field])}, expected #{inspect(value)}; converter will not overwrite DSEx evidence"
        )
      end
    end)

    unless is_map(get_in(dsex, ["results", "dsex_gepa"])) and
             is_map(get_in(dsex, ["metric_call_evidence", "observed"])) and
             positive_integer?(get_in(dsex, ["optimizer_budgets", "dsex_gepa"])) and
             is_map(get_in(dsex, ["seed_selection", "dsex_gepa"])) do
      Mix.raise("DSEx row #{family} is missing DSEx-owned research evidence")
    end
  end

  defp merge_comparator_evidence!(dsex, comparator_rows, campaign_id) do
    dsex =
      update_in(dsex, ["metric_call_evidence"], fn evidence ->
        Map.put_new(evidence, "upstream_sources", %{})
      end)

    Enum.reduce(comparator_rows, Map.put_new(dsex, "campaign_id", campaign_id), fn
  ...
  defp put_nested_new_or_equal!(row, path, value) do
    case get_in(row, path) do
      ...
      existing ->
        Mix.raise(
          "refusing to overwrite DSEx evidence at #{Enum.join(path, ".")}: #{inspect(existing)}"
        )
    end
  end
