defmodule Mix.Tasks.Imp.Identity.Evaluate do
  ...
  def run(args) do
    ...
    case Imp.IdentityEvaluation.compile(registry, assessments, flags, dissent, atlas, scenarios) do
      {:error, errors} ->
        Mix.raise("identity evaluation is invalid:\n" <> Enum.map_join(errors, "\n", &"- #{&1}"))

      {:ok, report} ->
        Imp.IdentityCheckpoint.write_atomic!(out, Jason.encode!(report, pretty: true) <> "\n")
  ...
  defp jsonl(opts, key, default, load_opts \\ []) do
    ...
    |> Imp.IdentityEvaluation.load_jsonl!(load_opts)
  end
  ...
  defp jsonls(opts, key, defaults, load_opts) do
    ...
    Enum.flat_map(paths, &Imp.IdentityEvaluation.load_jsonl!(&1, load_opts))
  end
