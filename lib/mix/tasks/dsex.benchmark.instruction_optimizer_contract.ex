defmodule Mix.Tasks.Dsex.Benchmark.InstructionOptimizerContract do
  @moduledoc """
  ...
  alias DSEx.Optimizer.{DemoCandidates, InstructionProposer, MIPROv2, SIMBA}
  alias DSEx.Optimizer.MIPROv2.Config
  alias DSEx.Optimizer.SIMBA.Buckets

  @shortdoc "Run matched DSEx/DSPy instruction-optimizer contracts"
  ...
  defp mipro_categorical_rows(mipro) do
    ...
    for {id, upstream_space, dsex_space} <- [
      ...
      actual_shape = categorical_shape(dsex_space, & &1)
  ...
  defp row(id, expected, actual, comparator \\ &Kernel.==/2) do
    ...
    %{
      ...
      "expected" => expected,
      "actual" => actual,
      "errors" => if(passing, do: [], else: ["DSEx result differs from pinned DSPy contract"])
    }
  end
  ...
  defp declared_native_deviations do
    [
      %{
        ...
        "dsex" => "joint categorical Parzen sampler",
      ...
      %{
        ...
        "dsex" => "retains the final call for each predictor",
      ...
      %{
        ...
        "dsex" => "native grounded prompt with program, data, and rotated demo context",
