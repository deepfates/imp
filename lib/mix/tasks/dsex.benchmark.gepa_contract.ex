defmodule Mix.Tasks.Dsex.Benchmark.GepaContract do
  @moduledoc """
  ...
  @shortdoc "Run matched DSEx/GEPA v0.1.1 structural contracts"
  ...
  defp named_mutation_row(upstream) do
    ...
    planner = DSEx.predict("question -> answer")
    writer = DSEx.predict("draft -> answer")
  ...
  defp row(id, expected, actual, comparator \\ &Kernel.==/2) do
    ...
    %{
      ...
      "expected" => expected,
      "actual" => actual,
      "errors" => if(passing, do: [], else: ["DSEx result differs from pinned GEPA contract"])
    }
  end
  ...
  defp declared_native_deviations do
    [
      ...
      %{
        ...
        "dsex" => "JSON checkpoint embeds the exported :rand state",
        "consequence" =>
          "result JSON round-trips in both; DSEx additionally resumes the exact native RNG stream"
      },
      %{
        "id" => "release_metadata_version",
        "gepa" => "tag v0.1.1 retains project version 0.1.0 in pyproject.toml",
        "dsex" => "contract identifies the release as 0.1.1",
        "consequence" =>
  ...
  defp current_gepa_python do
    System.get_env("DSEX_GEPA_V011_PYTHON") || System.get_env("DSEX_GEPA_PYTHON") ||
      "python3"
  end
  ...
  defp current_gepa_root do
    System.get_env("DSEX_GEPA_V011_ROOT") || System.get_env("DSEX_GEPA_ROOT") ||
      "tmp/gepa-v0.1.1"
  end
