defmodule Mix.Tasks.Imp.Benchmark.LiveMatrix do
  @moduledoc """
  Aggregate live matched-model campaign artifacts into a matrix report.

      mix imp.benchmark.live_matrix

  This task does not call providers. It consumes campaign artifacts emitted by
  `mix imp.benchmark.parity.aggregate` or `mix imp.benchmark.parity.campaign`
  ...
  defp expand_input(path) do
    if File.dir?(path) do
      path |> Path.join("imp-dspy-parity-campaign-*.json") |> Path.wildcard()
    else
      Path.wildcard(path)
    end
  end
  ...
  defp campaign_ids(opts) do
    ...
    |> add_csv(System.get_env("IMP_BENCH_CAMPAIGN_ID"))
    |> add_csv(System.get_env("IMP_BENCH_CAMPAIGN_IDS"))
    |> Enum.uniq()
  end
  ...
  defp unavailable_notes(opts) do
    ...
    note =
      Keyword.get(opts, :historical_unavailable_note) ||
        System.get_env("IMP_HISTORICAL_UNAVAILABLE_NOTE")
  ...
  defp failed_attempt_summary(artifacts) do
    ...
    %{
      ...
      "by_lane" => by_lane,
      "note" =>
        "Zero-accepted live attempts are retained as endpoint diagnostics but are not selected as Imp-vs-DSPy parity evidence."
    }
  end
  ...
  defp matrix_report(
    ...
    %{
      ...
      "summary" => %{
        ...
        "imp_instrumentation" => matrix_instrumentation_summary(models),
  ...
  defp model_row(artifact, max_age_hours) do
    ...
    %{
      ...
      "imp_instrumentation" => model_instrumentation_summary(artifact["tasks"] || []),
  ...
  defp generation_proof(%{"imp" => imp_generation, "dspy" => dspy_generation})
       when is_map(imp_generation) and is_map(dspy_generation) do
    imp_effective = imp_generation["effective"]
    dspy_effective = dspy_generation["effective"]

    prompt_contract =
      generation_prompt_contract(%{"imp" => imp_generation, "dspy" => dspy_generation})

    %{
      "requested_consistent" => true,
      "complete" => is_map(imp_effective) and is_map(dspy_effective),
      "matched" => is_map(imp_effective) and imp_effective == dspy_effective,
      "wire_api_matched" =>
        is_binary(imp_generation["wire_api"]) and
  ...
  defp generation_prompt_contract(%{"imp" => imp_generation, "dspy" => dspy_generation}) do
    case {imp_generation["prompt_contract"], dspy_generation["prompt_contract"]} do
      {imp, dspy} when is_binary(imp) and is_binary(dspy) ->
        %{"imp_req_llm" => imp, "python_dspy" => dspy}
  ...
  defp matrix_prompt_contract_summary(models) do
    ...
    %{
      ...
      "note" =>
        "Prompt-contract currency means the selected artifact was produced with the benchmark prompt/signature contract shipped by this Imp version. Obsolete prompt contracts are retained as historical evidence, not release evidence."
    }
  end
  ...
  defp matrix_instrumentation_summary(models) do
    instrumented =
      Enum.filter(models, &get_in(&1, ["imp_instrumentation", "coverage", "complete"]))

    shares =
      instrumented
      |> Enum.map(&get_in(&1, ["imp_instrumentation", "lm_duration_share", "mean"]))
    ...
    %{
      ...
      "note" =>
        "Complete Imp live instrumentation means the selected campaign artifact has instrumentation for every covered row in every covered task."
    }
  end
  ...
  defp model_instrumentation_summary(tasks) do
    summaries =
      tasks
      |> Enum.map(fn task -> {task["task"], task["imp_instrumentation"] || %{}} end)
  ...
  defp matrix_runtime_shape_summary(models) do
    ...
    message_ratios =
      shaped
      |> Enum.map(& &1["runtime_shape"]["message_chars_ratio_imp_over_dspy_mean"])
      |> Enum.filter(&is_number/1)

    raw_ratios =
      shaped
      |> Enum.map(& &1["runtime_shape"]["raw_chars_ratio_imp_over_dspy_mean"])
      |> Enum.filter(&is_number/1)
    ...
    %{
      ...
      "note" =>
        "Runtime shape compares Imp and DSPy prompt/output size instrumentation. It is diagnostic evidence for efficiency, not a byte-for-byte prompt compatibility requirement."
    }
  end
  ...
  defp model_runtime_shape_summary(tasks) do
    ...
    message_ratios =
      summaries
      |> Enum.map(fn {_task, summary} ->
        summary["message_chars_ratio_imp_over_dspy_mean"]
      end)
      |> Enum.filter(&is_number/1)

    raw_ratios =
      summaries
      |> Enum.map(fn {_task, summary} -> summary["raw_chars_ratio_imp_over_dspy_mean"] end)
      |> Enum.filter(&is_number/1)
    ...
    %{
      ...
      "message_chars_ratio_imp_over_dspy_mean" => average(message_ratios),
      "raw_chars_ratio_imp_over_dspy_mean" => average(raw_ratios),
      "by_task" => Map.new(summaries)
    }
  end
  ...
  defp runtime_shape_has_ratios?(%{} = runtime_shape) do
    is_number(runtime_shape["message_chars_ratio_imp_over_dspy_mean"]) or
      is_number(runtime_shape["raw_chars_ratio_imp_over_dspy_mean"])
  end
  ...
  defp matrix_latency_summary(models) do
      ...
      Map.new(models, fn model ->
        {
          ...
          %{
            ...
            "ratio_imp_over_dspy" => get_in(model, ["latency", "latency_ratio_imp_over_dspy"]),
            "imp_duration_ms" => get_in(model, ["latency", "imp_duration_ms"]),
    ...
    %{
      ...
      "by_model" => by_model,
      "note" =>
        "Latency parity is evaluated from the selected campaign aggregate for each model; transport records Imp ReqLLM pool settings when the run configured them explicitly."
    }
  end
  ...
  defp model_transport(%{"value" => %{"imp_transport" => transport}}) when is_map(transport),
    do: transport

  defp model_transport(%{"imp" => %{"transport" => transport}}) when is_map(transport),
    do: transport
  ...
  defp parity_summary(%{} = parity) do
    Map.take(parity, [
      ...
      "strict_task_gap",
      "latency_parity",
      "max_latency_ratio_imp_over_dspy"
    ])
  end
  ...
  defp observed_cost_estimate(usage, covered, expected) do
    ...
    %{
      ...
      "assumptions" => %{
        ...
        "runtime_usage" => Map.take(usage, ["imp", "dspy"])
      },
      "note" =>
        "Observed provider-reported Imp and DSPy usage/cost for accepted rows; remaining and full estimates scale the observed per-row averages linearly."
    }
  end
  ...
  defp modeled_cost_estimate(artifact, covered, expected) do
    ...
    input_per_example = env_int("IMP_BENCH_INPUT_TOKENS_PER_EXAMPLE", 1_500)
    output_per_example = env_int("IMP_BENCH_OUTPUT_TOKENS_PER_EXAMPLE", 128)
    ...
    input_usd_per_1m = env_float("IMP_BENCH_INPUT_USD_PER_1M")
    output_usd_per_1m = env_float("IMP_BENCH_OUTPUT_USD_PER_1M")
    ...
    %{
      ...
      "note" =>
        if(usd,
          do:
            "Estimated from covered, remaining, and full examples for Imp and DSPy runs using operator-supplied pricing env vars.",
          else:
            "Token-only estimate from covered, remaining, and full examples for Imp and DSPy runs. Set IMP_BENCH_INPUT_USD_PER_1M and IMP_BENCH_OUTPUT_USD_PER_1M from current provider pricing to include USD estimates."
        )
    }
  end
  ...
  defp error_summary(artifact) do
    ...
    |> Enum.map(fn task ->
      %{
        "task" => task["task"],
        "imp_errors" => length(task["imp_errors"] || []),
