defmodule Mix.Tasks.Dsex.Benchmark.RlmContract do
  @moduledoc """
  Run matched T1 operational contracts against DSEx RLM and current DSPy RLM.
  ...
  @shortdoc "Run matched DSEx/DSPy RLM operational contracts"
  ...
  def run(args) do
    ...
    dsex_rows = Enum.map(fixture["cases"], &run_dsex_case/1)
    dspy = run_dspy!(python, cases_path, out_dir)
    artifact = compare(fixture, dsex_rows, dspy)

    path =
      Path.join(out_dir, "rlm-operational-contract-#{timestamp_slug()}.json")
      |> DSEx.BenchmarkTruth.ArtifactFile.write_json!(artifact)
  ...
  defp run_dsex_case(%{"disposition" => "deviation"} = contract) do
    base_row(contract)
    |> Map.merge(%{
      "executed" => false,
      "passing" => true,
  ...
  defp run_dsex_case(contract) do
    ...
    result = DSEx.call(rlm, contract["inputs"])
    prompts = Agent.get(calls, &Enum.reverse/1)
    Agent.stop(calls)
    row = evaluate_dsex_result(contract, result, prompts)
  ...
  defp evaluate_dsex_result(contract, {:ok, prediction}, prompts) do
    ...
    output = DSEx.get(prediction, :answer)
  ...
  defp evaluate_dsex_result(contract, {:error, reason}, prompts) do
    base_row(contract)
    |> Map.merge(%{
      "executed" => true,
      "passing" => false,
  ...
  defp compare(fixture, dsex_rows, dspy) do
    dspy_by_id = Map.new(dspy["rows"], &{&1["id"], &1})

    rows =
      Enum.map(dsex_rows, fn dsex ->
        dspy_row = Map.fetch!(dspy_by_id, dsex["id"])

        passing =
          if dsex["disposition"] == "deviation" do
            dsex["passing"] and dspy_row["passing"]
          else
            dsex["passing"] and dspy_row["passing"] and
              dsex["status"] == dspy_row["status"] and
              dsex["output"] == dspy_row["output"] and
              dsex["subcalls"] == dspy_row["subcalls"] and
              get_in(dsex, ["trace", "entry_count"]) ==
                get_in(dspy_row, ["trace", "entry_count"])
          end

        %{
          "id" => dsex["id"],
          "invariant" => dsex["invariant"],
          "disposition" => dsex["disposition"],
          "required" => dsex["required"],
          "passing" => passing,
          "dsex" => dsex,
          "dspy" => dspy_row
        }
  ...
  defp current_dspy_python do
    System.get_env("DSEX_DSPY_CURRENT_PYTHON") || "tmp/dspy-parity-venv/bin/python"
  end

  defp current_dspy_env do
    case System.get_env("DSEX_DSPY_CURRENT_PYTHONPATH") do
      nil ->
