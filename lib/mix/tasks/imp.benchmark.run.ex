defmodule Mix.Tasks.Imp.Benchmark.Run do
  @moduledoc """
  Run benchmark truth evaluation over canonical JSONL files.

      mix imp.benchmark.run --gsm8k benchmarks/data/gsm8k-test-0-20.jsonl \\
        --hotpotqa benchmarks/data/hotpotqa-validation-0-20.jsonl --max-examples 20
      mix imp.benchmark.run --colors benchmarks/data/colors-test-0-6.jsonl
  ...
  @shortdoc "Run Imp benchmark truth evaluations"
  ...
  defp live_lm(:live, opts) do
    ...
    Imp.req_llm("openai:#{model}", api_key: api_key)
  end
