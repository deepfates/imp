defmodule Mix.Tasks.Imp.Benchmark.InstructionOptimizerExperiment do
  @moduledoc """
  Run or resume a matched Imp/DSPy instruction-optimizer experiment.
  ...
  @shortdoc "Run a matched Imp/DSPy instruction-optimizer preflight"
  ...
  def run(args) do
    ...
    experiment_opts =
      ...
      |> maybe_put(:imp_artifact, Keyword.get(opts, :imp_artifact))
      |> maybe_put(:dspy_artifact, Keyword.get(opts, :dspy_artifact))

    if Keyword.get(opts, :plan, false) do
      plan =
        Imp.BenchmarkTruth.InstructionOptimizerExperiment.plan!(
      ...
      result = Imp.BenchmarkTruth.InstructionOptimizerExperiment.run(experiment_opts)
  ...
  defp runtimes!("both"), do: [:imp, :dspy]
  defp runtimes!("imp"), do: [:imp]
