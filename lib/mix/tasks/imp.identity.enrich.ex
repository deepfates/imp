defmodule Mix.Tasks.Imp.Identity.Enrich do
  ...
  def run(args) do
    ...
    registry = Imp.IdentityEvaluation.load_jsonl!(registry_path)
    ...
    enrichments = Imp.IdentityEnrichment.baseline(registry, atlas, baseline_opts)

    body =
      Enum.map_join(enrichments, "\n", &Jason.encode!/1) <>
        if(enrichments == [], do: "", else: "\n")

    Imp.IdentityCheckpoint.write_atomic!(out, body)
