defmodule Mix.Tasks.Dsex.Benchmark.AutoEvaluationContract do
  ...
  def run(args) do
    ...
    manifest =
      Keyword.get(
        opts,
        :manifest,
        DSEx.BenchmarkTruth.AutoEvaluationContract.default_manifest()
      )

    case Keyword.fetch(opts, :validate) do
      {:ok, path} ->
        path
        |> DSEx.BenchmarkTruth.ArtifactFile.read_run_json!()
        |> DSEx.BenchmarkTruth.AutoEvaluationContract.validate_artifact!(manifest)

        Mix.shell().info("Valid auto-evaluation differential: #{path}")

      :error ->
        result =
          DSEx.BenchmarkTruth.AutoEvaluationContract.run!(
            manifest: manifest,
            output:
              Keyword.get(
                opts,
