defmodule Mix.Tasks.Dsex.EvidenceAuthorities do
  ...
  def run(args) do
    ...
    ledger = DSEx.EvidenceAuthorities.load!(ledger_path)
    current = File.read!(doc_path)

    expected =
      DSEx.EvidenceAuthorities.replace_summary!(
        current,
        DSEx.EvidenceAuthorities.render_family_summary(ledger)
      )
