defmodule Mix.Tasks.Dsex.Benchmark.Search do
  @moduledoc """
  Write provider-free shared inference-time search evidence.

      mix dsex.benchmark.search --out tmp/search-benchmark
  ...
  def run(args) do
    ...
    artifact =
      DSEx.BenchmarkTruth.Search.run(
    ...
    path = DSEx.BenchmarkTruth.ArtifactFile.write_json!(path, artifact)
