defmodule Mix.Tasks.Dsex.Benchmark.Overhead do
  @moduledoc """
  Run provider-free DSEx-vs-DSPy overhead benchmarks.

      mix dsex.benchmark.overhead
  ...
  @shortdoc "Run provider-free DSEx-vs-DSPy overhead benchmarks"
  ...
  def run(args) do
    ...
    dsex = dsex_report(iterations, warmup, batch_size)
    dspy = dspy_report(python(opts), out_dir, iterations, warmup, batch_size)
    report = comparison_report(dsex, dspy, max_ratio)
  ...
  defp dsex_report(iterations, warmup, batch_size) do
    cases = benchmark_cases()

    %{
      "schema_version" => 1,
      "runner" => "dsex-overhead",
  ...
  defp benchmark_cases do
    signature = DSEx.signature("question -> answer", "Answer the question.")
    response = "[[ ## answer ## ]]\nParis"
    ...
    [
      {"signature_parse", fn -> DSEx.Signature.ensure("question -> answer") end},
      {"adapter_format",
       fn ->
         DSEx.Adapter.Chat.format(signature, %{question: "What is the capital of France?"},
           demos: []
         )
       end},
      {"adapter_parse", fn -> DSEx.Adapter.Chat.parse(signature, response, []) end},
      {"schema_validate",
       fn -> DSEx.Schema.validate_fields(signature.outputs, %{answer: "Paris"}) end},
      {"evaluation_loop",
       fn -> examples |> DSEx.Evaluate.new(metric) |> DSEx.Evaluate.run(program) end},
      {"metric_normalization",
       fn ->
         prediction = DSEx.Prediction.new(%{answer: "Paris"})
      ...
      {"optimizer_trial_scheduling",
       fn ->
         ...
         |> DSEx.Optimizer.BootstrapFewShot.compile(program, Enum.take(examples, 2))
       end},
      ...
      {"cache_hit",
       fn ->
         DSEx.Cache.put(:overhead_hit, response)
         DSEx.Cache.get(:overhead_hit)
       end},
      {"cache_miss",
       fn ->
         key = {:overhead_miss, System.unique_integer([:positive])}
         DSEx.Cache.fetch_or_store(key, fn -> response end)
       end},
      {"concurrent_orchestration",
       fn ->
         1..32
         |> DSEx.Tasks.async_stream(fn value -> value * value end,
  ...
  defp example(index) do
    DSEx.example(question: "q#{index}", answer: "Paris")
  ...
  defp comparison_report(dsex, dspy, max_ratio) do
    ...
    cases =
      ...
      |> Enum.sort()
      |> Enum.map(fn id ->
        compare_case(id, dsex_cases[id], dspy_cases[id], max_ratio)
      end)
  ...
  defp compare_case(id, dsex, dspy, max_ratio) do
    ratio = Float.round(dsex["median_us"] / max(dspy["median_us"], 0.001), 4)

    %{
      "id" => id,
      "passing" => ratio <= max_ratio,
      "median_ratio_dsex_over_dspy" => ratio,
      "dsex" => dsex,
      "dspy" => dspy
    }
  end
