defmodule Mix.Tasks.Imp.Benchmark.FastSlow do
  @moduledoc """
  ...
  alias Imp.Training.FastSlow.{Checkpoint, Config, Runner, State}
  ...
  def build_artifact do
    ...
    artifact = %{
      "schema_version" => 1,
      "artifact_type" => "imp_fast_slow_protocol_campaign",
      ...
      "provenance" => %{
        ...
        "command" => "mix imp.benchmark.fast_slow --out <path>",
        "deterministic" => true,
  ...
  defmodule Backend do
    ...
    alias Imp.Training.FastSlow.{Config, DatasetState}
    ...
    def update_slow(state, batch, groups, intent, context) do
      ...
      if event["planned_failure"] do
        ...
        result_theta_id =
          Imp.Training.FastSlow.Theta.new!(state.cycle, payload, state.current_theta_id).id
    ...
    defp apply_slow_update(context, groups, intent, _theta) do
      session = Imp.Clients.ReinforcementSession.new(context["session"])
