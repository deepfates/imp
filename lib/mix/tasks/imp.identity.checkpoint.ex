defmodule Mix.Tasks.Imp.Identity.Checkpoint do
  @moduledoc """
  Validate identity generation portfolios and build the lossless registry.

      mix imp.identity.checkpoint
      mix imp.identity.checkpoint --build
      mix imp.identity.checkpoint --build --require-generation-floor
  """
  ...
  def run(args) do
    ...
    atlas = Imp.IdentityCheckpoint.load_atlas!(atlas_path)
    portfolios = Imp.IdentityCheckpoint.load_portfolios!(inbox_glob)

    case Imp.IdentityCheckpoint.compile(atlas, portfolios) do
      {:error, errors} ->
        Mix.raise("identity corpus is invalid:\n" <> Enum.map_join(errors, "\n", &"- #{&1}"))

      {:ok, %{events: events, report: report}} ->
        if Keyword.get(opts, :build, false) do
          Imp.IdentityCheckpoint.write_atomic!(
            registry_path,
            Imp.IdentityCheckpoint.render_registry(events)
          )
