defmodule Mix.Tasks.Dsex.Benchmark.OptimizeAnything do
  @moduledoc """
  Validate and emit Optimize Anything replication evidence.

      mix dsex.benchmark.optimize_anything --input path/to/rows.json --out benchmarks/results
      mix dsex.benchmark.optimize_anything --smoke --out tmp/optimize-anything
  ...
  alias DSEx.BenchmarkTruth.OptimizeAnything.{Artifact, Campaign}
  ...
  defp run_live(opts) do
    ...
    %{out_path: path} =
      Campaign.run(
        lm: DSEx.req_llm("#{provider}:#{model}"),
        provider: provider,
        model: model,
        seeds: seeds,
