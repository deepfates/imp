defmodule DSPy.Teleprompt.BootstrapFewShot do
  ...
  def compile(%__MODULE__{} = optimizer, program, trainset) do
    demos =
      ...
      |> Enum.filter(fn example ->
        inputs = example |> DSPy.Example.inputs() |> DSPy.Example.to_map()
  ...
  defp put_demos(%DSPy.Predict.Predict{} = program, demos),
    do: DSPy.Predict.Predict.with_demos(program, demos)

  defp put_demos(%DSPy.Predict.ChainOfThought{predict: predict} = program, demos),
    do: %{program | predict: DSPy.Predict.Predict.with_demos(predict, demos)}
