defmodule DSPy.Teleprompt.Report do
  ...
  defp put_metadata(%DSPy.Predict.Predict{metadata: metadata} = program, key, value) do
    %{program | metadata: Map.put(metadata, key, value)}
  end

  defp put_metadata(%DSPy.Predict.ChainOfThought{predict: predict} = program, key, value) do
    %{program | predict: put_metadata(predict, key, value)}
  end

  defp put_metadata(program, _key, _value), do: program

  defp get_metadata(%DSPy.Predict.Predict{metadata: metadata}, key), do: Map.get(metadata, key)

  defp get_metadata(%DSPy.Predict.ChainOfThought{predict: predict}, key),
