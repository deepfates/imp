defmodule DSPy.Settings do
  ...
  @name __MODULE__
  @defaults %{lm: nil, adapter: DSPy.Adapter.Chat, retriever: nil, callbacks: []}
  ...
  def get do
    ...
    Map.merge(global, Process.get(:dspy_context, %{}))
  end

  def fetch!(key), do: get() |> Map.fetch!(key)

  def context(opts, fun) when is_function(fun, 0) do
    previous = Process.get(:dspy_context, %{})
    Process.put(:dspy_context, Map.merge(previous, Map.new(opts)))

    try do
      fun.()
    after
      Process.put(:dspy_context, previous)
    end
  end
