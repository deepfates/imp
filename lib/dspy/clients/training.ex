defmodule DSPy.Clients.TrainingJob do
  ...
  def refresh(%__MODULE__{} = job) do
    ...
    with {:ok, %{status: status, body: response}} when status in 200..299 <-
           DSPy.HTTP.post(
             job.transport || DSPy.HTTP.Hackneyless,
             job.status_url,
             headers,
             body,
             []
...
defmodule DSPy.Clients.Trainer do
  ...
  @callback finetune(term(), DSPy.Clients.HTTPLM.t() | term(), list(DSPy.Example.t()), keyword()) ::
              {:ok, DSPy.Clients.TrainingJob.t()} | {:error, term()}
...
defmodule DSPy.Clients.HTTPTrainer do
  ...
  def finetune(%__MODULE__{} = trainer, lm, examples, opts) do
    ...
    with {:ok, %{status: status, body: response}} when status in 200..299 <-
           DSPy.HTTP.post(trainer.transport, trainer.submit_url, headers, body, opts),
         {:ok, decoded} <- Jason.decode(response) do
...
defmodule DSPy.Clients.OpenAITrainer do
  ...
  def new(opts \\ []) do
    ...
    DSPy.Clients.HTTPTrainer.new(:openai, String.trim_trailing(base, "/") <> "/fine_tuning/jobs",
      api_key: api_key,
      transport: Keyword.get(opts, :transport, DSPy.HTTP.Hackneyless),
  ...
  defp payload(lm, examples, opts) do
    %{
      model: Map.get(lm, :model),
      training_file: Keyword.get(opts, :training_file, "inline://dspy-elixir"),
      hyperparameters: Map.new(Keyword.get(opts, :hyperparameters, [])),
      dspy_training_data: Enum.map(examples, &DSPy.Example.to_map/1)
    }
  end
end

defmodule DSPy.Clients.DatabricksTrainer do
  ...
  def new(opts \\ []) do
    ...
    DSPy.Clients.HTTPTrainer.new(
      :databricks,
      String.trim_trailing(base, "/") <> "/api/2.0/dspy/finetune",
      api_key: api_key,
      transport: Keyword.get(opts, :transport, DSPy.HTTP.Hackneyless),
      status_url: String.trim_trailing(base, "/") <> "/api/2.0/dspy/finetune/{id}",
      payload_builder: &payload/3
    )
  end
  ...
  defp payload(lm, examples, opts) do
    %{
      ...
      train_data: Enum.map(examples, &DSPy.Example.to_map/1),
