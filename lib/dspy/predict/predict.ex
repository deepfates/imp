defmodule DSPy.Predict.Predict do
  @moduledoc "Basic DSPy module that maps signature inputs to outputs with an LM."
  ...
  def call(%__MODULE__{} = predict, inputs) when is_list(inputs) or is_map(inputs) do
    with {:ok, lm} <- require_lm(predict.lm),
         ...
         {:ok, raw} <- DSPy.LM.generate(lm, messages, predict.config),
  ...
  def with_signature(%__MODULE__{} = predict, signature),
    do: %{predict | signature: DSPy.Signature.ensure(signature)}
  ...
  defp add_trace(%DSPy.Prediction{} = prediction, messages, raw) do
