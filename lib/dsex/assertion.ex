defmodule DSEx.Assertion do
  @moduledoc """
  Named runtime constraint for DSEx predictions.
  ...
  feedback for another attempt.
      ...
      iex> assertion = DSEx.Assertion.new(:one_word, fn pred ->
      ...>   pred |> DSEx.Prediction.get(:answer, "") |> String.split() |> length() == 1
  ...
  def new(name, predicate, opts) when is_atom(name) or is_binary(name) do
    opts = DSEx.Options.validate!(opts, @option_schema, "DSEx.Assertion.new/3")
  ...
  def new(name, _predicate, _opts) do
    raise ArgumentError,
          "DSEx.Assertion.new/3 expects an atom or string name; got: #{inspect(name)}"
  end
  ...
  def normalize!(other) do
    raise ArgumentError,
          "expected DSEx.Assertion structs or {name, predicate, message} tuples; got: #{inspect(other)}"
  end
  ...
  defp validate_predicate!(predicate) do
    raise ArgumentError,
          "DSEx.Assertion predicate must be a unary or binary function; got: #{inspect(predicate)}"
  end
end
