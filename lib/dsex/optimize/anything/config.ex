defmodule DSEx.Optimize.Anything.Config do
  @moduledoc """
  ...
  DSEx deliberately does not: `reflection.reflection_lm` defaults to `nil` and
  ...
  defmodule Engine do
    ...
    alias DSEx.Optimize.Anything.Config.Persistence
    ...
    @spec new(keyword()) :: t()
    def new(opts \\ []) do
      values = DSEx.Options.validate!(opts, @schema, "#{inspect(__MODULE__)}.new/1")
  ...
  defmodule Merge do
    ...
    def new(opts \\ []) do
      opts
      |> DSEx.Options.validate!(@schema, "#{inspect(__MODULE__)}.new/1")
  ...
  defmodule Refiner do
    ...
    @schema [
      refiner_lm: [type: {:custom, DSEx.LM, :validate_lm, []}, default: nil],
  ...
  @type t :: %__MODULE__{
          ...
          tracking: Tracking.t(),
          stopper: term() | nil,
          callbacks: [DSEx.Optimizer.GEPA.Callback.callback()]
        }
  ...
  def to_map(%__MODULE__{} = config) do
    ...
    %{
      "type" => "dsex_optimize_anything_config",
      "schema_version" => 1,
