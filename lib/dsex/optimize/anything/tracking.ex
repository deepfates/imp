defmodule DSEx.Optimize.Anything.Tracking do
  @moduledoc false
  ...
  alias DSEx.Tracking.{MLflow, Session}
  alias DSEx.Tracking.WandB.Backend, as: WandBBackend

  @spec open(Config.Tracking.t()) :: {DSEx.Optimizer.GEPA.Callback.callback(), Session.t()}
      ...
      {:error, reason} ->
        raise RuntimeError,
              "Optimize Anything tracking could not start: " <>
                inspect(DSEx.Redaction.redact(reason))
    end
  end

  @spec close(Session.t(), DSEx.Tracking.Backend.status()) :: :ok | {:error, term()}
    ...
    specs ++
      [
        {WandBBackend,
         [
           api_key: api_key,
           transport: DSEx.Tracking.Transport.Req,
  ...
  defp default_wandb_project(init) do
    if Map.has_key?(init, :project) or Map.has_key?(init, "project"),
      do: init,
      else: Map.put(init, :project, "dsex-optimize-anything")
  end
end
