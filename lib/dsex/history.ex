defmodule DSEx.History do
  @moduledoc """
  Immutable conversation history for signature-shaped DSEx programs.

  `DSEx.History` mirrors the DSPy `History` primitive philosophically: history is
  ...
  message schema.
      ...
      iex> history =
      ...>   DSEx.History.new()
      ...>   |> DSEx.History.append(%{question: "Capital of France?", answer: "Paris"})
      iex> DSEx.History.messages(history)
  ...
  def new(messages) do
    raise ArgumentError,
          "DSEx.History.new/1 expects a list of field maps; got: #{inspect(messages)}"
  end
  ...
  def append(history, turn) do
    raise ArgumentError,
          "DSEx.History.append/2 expects a DSEx.History and field map; got: #{inspect(history)} and #{inspect(turn)}"
  end
  ...
  def redact(%__MODULE__{messages: messages} = history, keys \\ DSEx.Redaction.default_keys()) do
    %{history | messages: DSEx.Redaction.redact(messages, keys)}
  end
  ...
  def dump(%__MODULE__{messages: messages}) do
    %{
      "type" => "history",
      "messages" => DSEx.Optimizer.Report.json_safe(messages)
    }
  end
  ...
  def load(state) do
    raise ArgumentError,
          "DSEx.History.load/1 expects a history map with list \"messages\"; got: #{inspect(state)}"
  end
  ...
  defp normalize_turn!(turn) do
    raise ArgumentError,
          "DSEx.History messages must be field maps or field pair lists; got: #{inspect(turn)}"
  end
end
