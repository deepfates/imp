defmodule DSEx.ProgramParameters do
  ...
  alias DSEx.Optimizer.Parameter.Set
  ...
  @keyword_marker "$dsex_optimizer_keyword"
  ...
  def snapshot(%_module{} = program) do
    ...
    case DSEx.ProgramAccess.get_metadata(program, @parameter_state_key) do
  ...
  def apply_changes_with_snapshot(%_module{} = program, changes) when is_list(changes) do
    ...
    with {:ok, normalized_changes} <- normalize_changes(changes),
      ...
      updated = DSEx.ProgramAccess.put_metadata(updated, @parameter_state_key, committed)
  ...
  defp playbook_descriptors(program) do
    Enum.map(playbooks(program), fn %{name: name, playbook: playbook} ->
      %{
        ...
        kind: :playbook,
        value: DSEx.Playbook.dump(playbook),
  ...
  defp prepare_change(%Change{kind: :playbook, value: value} = change, descriptor) do
    {:ok, {change, descriptor, DSEx.Playbook.load!(value)}}
  rescue
    error in ArgumentError -> {:error, Exception.message(error)}
  end
  ...
  defp demo_value!(%DSEx.Example{} = demo) do
    %{
      "demos" => Enum.map(demo.demos, &demo_value!/1),
      "fields" => runtime_json_value!(DSEx.Example.to_map(demo), "demo fields"),
      "input_keys" => Enum.map(demo.input_keys || [], &json_key!(&1, "demo input key"))
    }
  end

  defp demo_value!(demo), do: demo |> DSEx.Example.new() |> demo_value!()
  ...
  defp demo_from_value!(
    ...
    demo = DSEx.Example.new(fields)
    demo = if input_keys == [], do: demo, else: DSEx.Example.with_inputs(demo, input_keys)
    DSEx.Example.with_demos(demo, Enum.map(demos, &demo_from_value!/1))
  end
  ...
  defp runtime_to_json!(%DSEx.Example{} = value, _context), do: demo_value!(value)
  ...
  defp update_tool(program, _name, _update) do
    raise ArgumentError,
          "tool parameters are only available for DSEx.Predict.ReAct and DSEx.Predict.ReActV2, got: #{inspect(program.__struct__)}"
  end
