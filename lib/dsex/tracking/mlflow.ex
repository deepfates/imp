defmodule DSEx.Tracking.MLflow do
  ...
  @doc "Logs summary values as `dsex.summary.*` run tags in one log-batch request."
    ...
    with {:ok, entries} <- key_value_entries(summary),
         tags <-
           Enum.map(entries, fn {key, value} ->
             %{"key" => "dsex.summary.#{key}", "value" => stringify(value)}
           end),
  ...
  defp transport(opts) do
    transport = Keyword.get(opts, :transport, DSEx.Tracking.Transport.Req)
