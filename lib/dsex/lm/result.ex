defmodule DSEx.LM.Result do
  ...
  @atom_output :__dsex_lm_output__
  @atom_metadata :__dsex_lm_metadata__
  @string_output "__dsex_lm_output__"
  @string_metadata "__dsex_lm_metadata__"
  @reserved_keys [@atom_output, @atom_metadata, @string_output, @string_metadata]

  @type output :: binary() | map() | DSEx.Prediction.t()
  @type metadata :: map()

  @spec split(output()) :: {:ok, output(), metadata()} | {:error, term()}
  def split(value) when is_binary(value) or is_struct(value, DSEx.Prediction),
    do: {:ok, value, %{}}
