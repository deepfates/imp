defmodule DSEx.Playbook.Policy do
  @moduledoc """
  Hard limits and admission rules for a `DSEx.Playbook`.
  ...
  def validate_content(content, %__MODULE__{} = policy) when is_binary(content) do
    cond do
      ...
      policy.reject_secrets and DSEx.Redaction.redact(content) != content ->
  ...
  def validate_section(section, %__MODULE__{} = policy) when is_binary(section) do
    cond do
      ...
      policy.reject_secrets and DSEx.Redaction.redact(section) != section ->
