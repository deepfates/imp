defmodule DSEx.BenchmarkTruth.RLMStatistics do
  ...
  def aggregate(rows, manifest) do
    ...
    %{
      ...
      "bootstrap" => %{
        "method" => "paired nonparametric bootstrap over shared logical row keys",
  ...
  defp paired_comparisons(rows, manifest) do
    ...
    Enum.flat_map(groups, fn {runtime, runtime_rows} ->
      ...
      for left <- DSEx.BenchmarkTruth.RLMManifest.approach_ids(),
          right <- DSEx.BenchmarkTruth.RLMManifest.approach_ids(),
          left < right,
          Map.has_key?(by_approach, left),
          Map.has_key?(by_approach, right) do
        paired(runtime, left, by_approach[left], right, by_approach[right], manifest)
      end
  ...
  defp paired(runtime, left_name, left_rows, right_name, right_rows, manifest) do
    ...
    diffs = Enum.map(keys, fn key -> left[key]["score"] - right[key]["score"] end)
  ...
  defp pair_key(row), do: {row["family"], row["example_id"]}
