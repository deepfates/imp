defmodule DSEx.BenchmarkTruth.HoverFeedback do
  @moduledoc false
  ...
  @doc "Returns callbacks keyed by the upstream program's DSEx predictor names."
  ...
  def callbacks_for(program) when is_struct(program) do
    actual = program |> DSEx.ProgramParameters.predictors() |> Enum.map(& &1.name)
  ...
  defp example_value(%DSEx.Example{} = example, key, default),
    do: DSEx.Example.get(example, key, default)

  defp example_value(example, key, default), do: value(example, key) || default

  defp prediction_value(%DSEx.Prediction{} = prediction, key, default),
    do: DSEx.Prediction.get(prediction, key, default)
