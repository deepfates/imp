defmodule DSEx.BenchmarkTruth.HotpotFeedback do
  @moduledoc false
  ...
  defp example_data!(%ComponentFeedback{example: %DSEx.Example{} = example}) do
    fields = DSEx.Example.to_map(example)
  ...
  defp example_data!(%ComponentFeedback{example: other}) do
    invalid!("example must be a DSEx.Example, got: #{inspect(other)}")
  end

  defp program_outputs!(%ComponentFeedback{program_output: %DSEx.Prediction{} = prediction}),
    do: DSEx.Prediction.to_map(prediction)

  defp program_outputs!(%ComponentFeedback{program_output: other}) do
    invalid!("program_output must be a DSEx.Prediction, got: #{inspect(other)}")
  end
  ...
  defp exact_match!(prediction, answer) when is_binary(prediction),
    do: DSEx.Metrics.em(prediction, answer)
