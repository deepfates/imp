defmodule DSEx.BenchmarkTruth.InstructionOptimizerExperiment do
  @moduledoc false

  alias DSEx.BenchmarkTruth.{ArtifactFile, InstructionOptimizerCampaign, RunContext}
  ...
  def derive!(source, opts \\ []) do
    ...
    check_dsex_pin!(manifest, context)
    sources = Map.put(manifest["source_commits"], "dsex", context.source_commits["dsex"])
  ...
  def run(opts) do
    ...
    runtimes = runtimes!(Keyword.get(opts, :runtimes, [:dsex, :dspy]))

    dsex =
      if :dsex in runtimes,
        do: run_dsex!(derived, opts),
        else: artifact(Keyword.get(opts, :dsex_artifact))
    ...
    merged =
      if dsex && dspy do
        report = merge_outputs!(derived, dsex, dspy)
  ...
  def merge_outputs!(derived, dsex_source, dspy_source, _opts \\ []) do
    dsex = artifact(dsex_source) || raise ArgumentError, "DSEx artifact is required"
    dspy = artifact(dspy_source) || raise ArgumentError, "DSPy artifact is required"
    verify_dsex!(dsex, derived)
    verify_dspy!(dspy, derived)

    %{
      "schema_version" => 1,
      "runner" => "dsex-dspy-instruction-optimizer-experiment",
  ...
  defp models!(raw) do
    ...
    case model do
      value when is_binary(value) ->
        %{
          "logical" => value,
          "dsex" => string!(bindings["dsex"] || value, "runtime_models.dsex"),
      ...
      %{} ->
        %{
          ...
          "dsex" => string!(model["dsex"], "model.dsex"),
          "dspy" => string!(model["dspy"], "model.dspy")
        }

      _ ->
        raise ArgumentError, "model must be a string or logical/dsex/dspy object"
    end
  end
  ...
  defp dataset!(raw, manifest_path, family) do
    ...
    Enum.each(
      ...
      &require!(
        ...
        "dataset #{&1} path is incompatible with DSEx"
      )
    )
  ...
  defp authority!(raw, opts) do
    registry =
      DSEx.UpstreamAuthorityRegistry.load!(
        Keyword.get(opts, :authority_registry_path, DSEx.UpstreamAuthorityRegistry.path())
      )

    authority = DSEx.UpstreamAuthorityRegistry.authority!(registry, @contract_id)
  ...
  defp run_context!(manifest, opts) do
    Keyword.get(opts, :run_context) ||
      RunContext.capture_git!(
        ...
        source_commits: Map.delete(manifest["source_commits"], "dsex")
      )
  end

  defp check_dsex_pin!(manifest, context) do
    pin = manifest["source_commits"]["dsex"]

    require!(
      is_nil(pin) || pin == "resolved from campaign git_sha" ||
        pin == context.source_commits["dsex"],
      "source_commits.dsex mismatch"
    )
  end
  ...
  defp dsex_options(manifest, sources, context, out, checkpoints) do
    ...
    [
      ...
      campaign_id: manifest["campaign_id"],
      family: manifest["family"],
      model: manifest["models"]["dsex"],
      seed: manifest["seed"],
      arms: Enum.map(manifest["arms"], &Map.fetch!(@dsex_names, &1)),
  ...
  defp run_dsex!(derived, opts) do
    case Keyword.get(opts, :dsex_executor) do
      fun when is_function(fun, 1) ->
        artifact(fun.(maybe_lm(derived.dsex_options, opts)))
      ...
      nil ->
        ...
        lm =
          Keyword.get(opts, :lm) ||
            DSEx.req_llm(derived.manifest["models"]["dsex"],
              api_key: key,
        ...
        derived.dsex_options
        |> Keyword.put(:lm, lm)
        |> InstructionOptimizerCampaign.run()
        |> artifact()

      _ ->
        raise ArgumentError, "dsex_executor must accept one argument"
    end
  end
  ...
  defp verify_dsex!(a, d) do
    id = a["identity"] || %{}
    expected_budget = d.dsex_options |> Keyword.fetch!(:budget) |> json()

    checks = [
      a["runner"] == "dsex-instruction-optimizer-campaign",
      ...
      id["model"] == d.identity["models"]["dsex"],
    ...
    require!(Enum.all?(checks), "DSEx artifact identity or completion mismatch")
  end
  ...
  defp comparisons(dsex, dspy) do
    %{"dsex" => baseline_deltas(dsex_scores(dsex)), "dspy" => baseline_deltas(dspy_scores(dspy))}
  end
