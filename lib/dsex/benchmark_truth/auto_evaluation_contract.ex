defmodule DSEx.BenchmarkTruth.AutoEvaluationContract do
  @moduledoc false
  ...
  defp run_case!(%{"evaluator" => "semantic_f1"} = spec) do
    ...
    row(spec, DSEx.Prediction.get(result, :f1), result.score, length(prompts), prompts)
  end
  ...
  defp inputs(%{"input_shape" => "example_prediction"} = spec) do
    maybe_trace(
      %{
        example: DSEx.example(question: "What is the answer?", response: "gold answer"),
        pred: DSEx.prediction(response: "system answer", context: "retrieved context")
      },
      spec["trace"]
    )
  end
