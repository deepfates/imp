defmodule DSEx.BenchmarkTruth.HotpotMultiHop do
  @moduledoc false
  ...
  @doc "Builds the program around any retriever supported by `DSEx.Retrieve`."
  ...
  def call(%__MODULE__{} = program, inputs) when is_map(inputs) or is_list(inputs) do
    with {:ok, question} <- question(inputs),
      ...
      {:ok, DSEx.Prediction.new(answer: answer, hop1_docs: hop1_docs, hop2_docs: hop2_docs)}
    end
  end
  ...
  defp retrieve(program, query, hop) do
    result =
      case program.retriever do
        ...
        retriever -> DSEx.Retrieve.retrieve(retriever, query, k: program.k)
      end
  ...
  defp predict(component, inputs, stage) do
    ...
    case DSEx.Module.call(component, inputs) do
      {:ok, prediction} ->
        case DSEx.Prediction.get(prediction, output) do
