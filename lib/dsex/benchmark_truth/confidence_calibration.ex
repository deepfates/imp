defmodule DSEx.BenchmarkTruth.ConfidenceCalibration do
  ...
  defp evaluate(rows, prompt_id, task, model, api_key, concurrency) do
    ...
    signature = DSEx.signature("text -> label: enum[#{enum}]", prompt)

    program =
      DSEx.predict(signature,
        lm: DSEx.req_llm(model, api_key: api_key, temperature: 0, max_tokens: 32),
  ...
  defp verify_provenance!("support_routing_v1", _path, _data_sha256, _rows, _opts) do
    %{
      verified?: false,
      reason: "DSEx-authored narrow fixture has no external label provenance"
    }
  end
