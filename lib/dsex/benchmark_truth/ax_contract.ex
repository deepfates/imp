defmodule DSEx.BenchmarkTruth.AxContract do
  @moduledoc false

  alias DSEx.Agent.Runtime
  alias DSEx.BenchmarkTruth.{ArtifactFile, RunContext}
  alias DSEx.Optimizer.GEPA.ModuleSelector
  ...
  def compare(%{"cases" => upstream}) do
    ...
    [
      matched_row("typed_signature", upstream, dsex),
      matched_row("structured_output", upstream, dsex),
      matched_row("streaming_optional", upstream, dsex),
      tool_row(upstream, dsex),
      matched_row("usage", upstream, dsex),
      optimizer_row(upstream, dsex)
    ]
  end
  ...
  defp dsex_cases do
    ...
    tool = DSEx.Tool.new(:lookup, "Lookup a value", &%{found: true, key: &1.key})
    tool_result = DSEx.Tool.call(tool, %{key: "alpha"})

    agent =
      DSEx.Agent.new(:fixture, fn _inputs, runtime -> {:ok, :ok, runtime} end, tools: [tool])

    {:error, {:unknown_tool, :missing}, _runtime} =
      DSEx.Agent.call_tool(agent, :missing, %{}, Runtime.new())
  ...
  defp streaming_optional do
    ...
    {:ok, omitted_values} = DSEx.Adapter.Chat.parse(signature, omitted, [])
    {:ok, present_values} = DSEx.Adapter.Chat.parse(signature, present, [])
  ...
  defp collect_chunks(acc) do
    ...
    after
      1_000 -> raise "DSEx incremental stream listener did not terminate"
    end
  end
  ...
  defp matched_row(id, upstream, dsex) do
    upstream_value = upstream[id]
    dsex_value = dsex[id]

    # Ax exports input and output JSON schema together; DSEx intentionally exports
    # only output schema at the adapter boundary.
    comparable =
      if id == "structured_output", do: ax_output_schema(upstream_value), else: upstream_value

    row(id, "matched", comparable == dsex_value, upstream_value, dsex_value, nil)
  end

  defp tool_row(upstream, dsex) do
    ax = upstream["tools"]
    native = dsex["tools"]
    ...
    row(
      ...
      ax,
      native,
      "Both preserve native object meaning and reject unknown tools; Ax exposes a retryable validation error while DSEx returns a fail-fast tagged error."
    )
  end

  defp optimizer_row(upstream, dsex) do
    ax = upstream["optimizer_selection"]

    passing =
      ax["pickAtHalf"] == "stale::instruction" and map_size(ax["snapshot"]) == 2 and
        dsex["optimizer_selection"]["componentOrder"] == [:recent, :stale]
    ...
    row(
      ...
      passing,
      ax,
      dsex["optimizer_selection"],
      "Ax ships an adaptive stagnation-weighted component bandit. DSEx ships deterministic round-robin/all policies and a BEAM callback boundary for custom selectors; Ax is a comparator, not scientific authority."
    )
  end
  ...
  defp row(id, status, passing, upstream, dsex, deviation) do
    %{
      "id" => id,
      "required" => true,
      "status" => status,
