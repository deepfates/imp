defmodule DSEx.Optimizer.DemoCandidates do
  @moduledoc false

  alias DSEx.Optimizer.{Sampling, TrajectoryRunner}

  @spec build(struct(), [DSEx.Example.t()], function(), keyword()) ::
          {%{required(DSEx.ProgramParameters.name()) => [[DSEx.Example.t()]]}, map()}
  ...
  defp final_demo(%{
         example: %DSEx.Example{} = example,
         prediction: %DSEx.Prediction{} = prediction
       }) do
    augmented_demo(
      example |> DSEx.Example.inputs() |> DSEx.Example.to_map(),
  ...
  defp same_example?(example, demos) do
    fields = example |> normalize_example() |> DSEx.Example.to_map()
    Enum.any?(demos, &(DSEx.Example.to_map(&1) == fields))
  end
