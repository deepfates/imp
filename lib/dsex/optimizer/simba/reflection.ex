defmodule DSEx.Optimizer.SIMBA.Reflection do
  @moduledoc false
  ...
  def run(prompt_lm, payload) do
    ...
    program = DSEx.Predict.Predict.new(signature, lm: prompt_lm, adapter: DSEx.Adapter.JSON)

    with {:ok, prediction} <-
           DSEx.Predict.Predict.call(program, reflection_inputs(payload)),
         advice when is_map(advice) <- DSEx.get(prediction, :module_advice) do
      {:ok, advice, DSEx.get(prediction, :discussion)}
    else
