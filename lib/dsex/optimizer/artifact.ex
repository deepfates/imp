defmodule DSEx.Optimizer.Artifact do
  @moduledoc """
  Durable champion/challenger lifecycle for portable optimizer outputs.

  Artifacts contain checksummed `DSEx.Saving` program states and redacted
  ...
  alias DSEx.{ProgramParameters, Redaction, Saving}
  ...
  def apply(artifact, program, selection \\ :champion, opts \\ []) do
    ...
    Enum.reduce(target, program, fn {identity, %{name: target_name, predictor: target_predictor}},
      ...
      ProgramParameters.update_predictor(acc, target_name, fn live_predictor ->
        live_predictor
        |> DSEx.Predict.Predict.with_signature(source_predictor.signature)
        |> DSEx.Predict.Predict.with_demos(source_predictor.demos)
  ...
  defp parameters(candidate, opts) do
    ...
    |> Map.new(fn {identity, %{predictor: predictor}} ->
      {identity,
       %{
         "signature" => DSEx.Signature.dump(predictor.signature),
  ...
  defp ensure_predictors!(program) do
    if ProgramParameters.predictors(program) == [] do
      raise ArgumentError,
            "optimizer artifact candidates must expose at least one named predictor through DSEx.ProgramParameters"
    end
  end
  ...
  defp validate_signature_compatibility!(left, right, identity) do
    shape = fn signature -> signature |> DSEx.Signature.dump() |> Map.delete("instructions") end
  ...
  defp security_proof do
    %{
      ...
      "json_safe" => true,
      "redaction_policy" => "dsex_default_v1"
    }
  end
