defmodule DSEx.Optimizer.Playbook.Campaign do
  @moduledoc false
    ...
    fn program, rows, context ->
      ...
      result =
        ...
        |> Enum.reduce_while({:ok, []}, fn {row, index}, {:ok, trajectories} ->
          ...
          case DSEx.call(program, %{equation: row["input"]}) do
            {:ok, prediction} ->
              answer = DSEx.get(prediction, :answer, "")
  ...
  defp artifact(config, dataset, splits, result, budget, api_key) do
    ...
    artifact = %{
      ...
      "limitations" => [
        ...
        "DSEx adopts the incremental playbook idea while intentionally using typed atomic deltas and held-out promotion rather than reproducing upstream string rewriting."
      ]
    }
