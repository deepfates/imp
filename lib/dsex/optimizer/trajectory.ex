defmodule DSEx.Optimizer.Trajectory do
  ...
  @spec project(runtime(), t() | DSEx.Prediction.t() | map(), keyword()) :: t()
  ...
  def validate!(other),
    do: invalid!("expected a DSEx.Optimizer.Trajectory, got: #{inspect(other)}")
  ...
  def redact(%__MODULE__{} = trajectory, keys \\ DSEx.Redaction.default_keys()) do
  ...
  def dump(%__MODULE__{} = trajectory) do
    ...
    %{
      "type" => "dsex_optimizer_trajectory",
  ...
  @spec load!(map()) :: t()
  def load!(%{"type" => "dsex_optimizer_trajectory", "schema_version" => @schema_version} = state) do
  ...
  defp projection_base(%DSEx.Prediction{} = prediction, opts) do
    %__MODULE__{
  ...
  defp projection_trace(%DSEx.Prediction{} = prediction), do: prediction_trace(prediction)
  defp projection_trace(value) when is_map(value), do: fetch(value, :trace, [])

  defp prediction_trace(%DSEx.Prediction{metadata: metadata} = prediction) do
  ...
  defp encode_term(%DSEx.Example{} = example) do
    %{
      "__trajectory_type__" => "example",
      "fields" => encode_term(DSEx.Example.to_map(example)),
      "input_keys" => encode_term(example.input_keys),
      "demos" => encode_term(example.demos)
    }
  end

  defp encode_term(%DSEx.Prediction{} = prediction) do
    %{
      "__trajectory_type__" => "prediction",
      "fields" => encode_term(DSEx.Prediction.to_map(prediction)),
  ...
  defp encode_term(%module{} = value)
       when module in [
              ...
              DSEx.Adapters.Types.Audio,
              DSEx.Adapters.Types.File,
              DSEx.Adapters.Types.Document,
              DSEx.Adapters.Types.Code,
              DSEx.Adapters.Types.Reasoning,
  ...
  defp decode_term(
    ...
    DSEx.Prediction.new(decode_term(fields),
  ...
  defp typed_modules do
    %{
      ...
      "audio" => DSEx.Adapters.Types.Audio,
      "file" => DSEx.Adapters.Types.File,
      "document" => DSEx.Adapters.Types.Document,
      "code" => DSEx.Adapters.Types.Code,
      "reasoning" => DSEx.Adapters.Types.Reasoning,
      "tool_call" => DSEx.Adapters.Types.ToolCall,
      "tool_result" => DSEx.Adapters.Types.ToolResult,
      "failure" => Failure
    }
  end

  defp redact_value(%DSEx.Adapters.Types.Image{} = value, keys),
    do: redact_attachment(value, [:data], keys)

  defp redact_value(%DSEx.Adapters.Types.Audio{} = value, keys),
    do: redact_attachment(value, [:data], keys)

  defp redact_value(%DSEx.Adapters.Types.File{} = value, keys),
    do: redact_attachment(value, [:data], keys)
  ...
  defp redact_value(value, keys) when is_map(value) do
    Map.new(value, fn {key, nested} ->
      probe = DSEx.Redaction.redact(%{key => :visible}, keys)
  ...
  defp redact_value(value, keys), do: DSEx.Redaction.redact(value, keys)
  ...
  defp maybe_inputs(example, keys), do: DSEx.Example.with_inputs(example, keys)
  defp maybe_demos(example, []), do: example
  defp maybe_demos(example, demos), do: DSEx.Example.with_demos(example, demos)
