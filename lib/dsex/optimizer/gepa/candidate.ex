defmodule DSEx.Optimizer.GEPA.Candidate do
  @moduledoc """
  Named textual components evaluated by a GEPA adapter.

  Component names use the same identity rules as `DSEx.ProgramParameters`.
  The helpers in this module provide the bridge between a DSEx program and the
  ...
  def from_program(program) do
    case ProgramParameters.predictors(program) do
      [] ->
        case DSEx.Optimizer.InstructionSearch.current_instruction(program) do
  ...
  @doc "Applies a complete named candidate to a program through `DSEx.ProgramParameters`."
