defmodule DSEx.Optimizer.GEPA.Result do
  @moduledoc """
  ...
  trajectories are also batch-aligned and reuse `DSEx.Optimizer.Trajectory`;
  ...
  defp validate_trajectories!(trajectories, component_names, batch_size, capture_traces)
    ...
    Enum.each(trajectories, fn {name, aligned} ->
      ...
      unless Enum.all?(aligned, &(is_nil(&1) or match?(%Trajectory{}, &1))) do
        raise ArgumentError, "GEPA component trajectories must reuse DSEx.Optimizer.Trajectory"
      end
    end)
  end
