defmodule DSEx.Observability do
  ...
  @default_trace_events [
    ...
    [:dsex, :training, :submit, :stop],
    [:dsex, :training, :submit, :exception],
    [:dsex, :training, :refresh, :start],
    [:dsex, :training, :refresh, :stop],
    [:dsex, :training, :refresh, :exception],
    [:dsex, :training, :cancel, :start],
    [:dsex, :training, :cancel, :stop],
    [:dsex, :training, :cancel, :exception]
  ]
  ...
  def inspect_artifact(value, opts \\ []) do
    opts =
      DSEx.Options.validate!(
        opts,
        @inspection_schema,
        "DSEx.Observability.inspect_artifact/2"
      )
  ...
  def handle_trace_event(event, measurements, metadata, agent) do
    event = {event, DSEx.Redaction.redact(measurements), DSEx.Redaction.redact(metadata)}
  ...
  def handle_optimizer_event(event, measurements, metadata, owner) do
    measurements = DSEx.Redaction.redact(measurements)
  ...
  defp normalize_inspection(%DSEx.History{} = history) do
    entries = Enum.map(DSEx.History.messages(history), &{:conversation, &1})
    {:history, :ok, %{turn_count: length(entries)}, entries}
  end

  defp normalize_inspection(%DSEx.Prediction{} = prediction) do
    metadata = prediction.metadata
    provider = map_entries(Map.get(metadata, :trace), :provider)
    tools = prediction |> DSEx.Prediction.get(:history) |> history_entries(:tool)
  ...
  defp normalize_inspection(%DSEx.Streaming.Messages.StatusMessageProvider{messages: messages}) do
    entries = list_entries(messages, :status)
    {:status_stream, :ok, %{message_count: length(entries)}, entries}
  end

  defp normalize_inspection(%DSEx.Streaming.Messages.StatusMessage{} = message) do
  ...
  defp normalize_inspection(value) do
    raise ArgumentError,
          "DSEx.Observability.inspect_artifact/2 does not recognize: #{safe_inspect(value)}"
  end
  ...
  defp history_entries(%DSEx.History{} = history, source),
    do: list_entries(DSEx.History.messages(history), source)
  ...
  defp prediction_status(%DSEx.Prediction{} = prediction) do
    case DSEx.Prediction.get(prediction, :termination_reason) do
  ...
  defp normalize_status(%DSEx.Prediction{} = prediction) do
  ...
  defp normalize_status(value) do
    raise ArgumentError,
          "DSEx.Observability.status/1 does not recognize: #{safe_inspect(value)}"
  end
  ...
  defp redact_status(%Status{} = status) do
    %{
      status
      | metadata: DSEx.Redaction.redact(status.metadata),
        message: DSEx.Redaction.redact(status.message)
    }
  end

  defp safe_inspect(value),
    do: value |> DSEx.Redaction.redact() |> redact_error_tuples() |> Kernel.inspect()

  defp redact_error_tuples(value) when is_tuple(value) do
    value
    |> Tuple.to_list()
    |> Enum.map(&(DSEx.Redaction.redact(&1) |> redact_error_tuples()))
    |> List.to_tuple()
  end
