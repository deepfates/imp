defmodule DSEx.Observability.Inspection do
  @moduledoc """
  ...
  def new(kind, status, summary, entries, opts) do
    ...
    entries =
      ...
      |> Enum.map(fn {{source, payload}, sequence} ->
        payload = if redact?, do: DSEx.Redaction.redact(payload), else: payload
        %{source: source, sequence: sequence, payload: compact(payload, max_bytes)}
      end)

    summary = if redact?, do: DSEx.Redaction.redact(summary), else: summary
