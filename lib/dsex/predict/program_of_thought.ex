defmodule DSEx.Predict.ProgramOfThought do
  ...
  Program execution uses `DSEx.Sandbox`; generated source is never passed to
  ...
  alias DSEx.{Prediction, Signature}
  ...
  def eval_program(program, inputs) do
    DSEx.Sandbox.eval(program, inputs)
  rescue
  ...
  defp retry_or_fail(pot, inputs, generated, reason, iteration, trace) do
    ...
    if iteration < effective_max_iters(pot) do
      ...
      # Keep the established DSEx error contract while still doing every
