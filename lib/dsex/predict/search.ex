...
defmodule DSEx.Predict.Search do
  ...
  def run(candidates, evaluator, opts)
    ...
    opts = DSEx.Options.validate!(opts, @option_schema, "DSEx.Predict.Search.run/3")
    ...
    DSEx.Telemetry.span([:dsex, :predict, :search], metadata, fn ->
  ...
  def run(candidates, evaluator, _opts) do
    raise ArgumentError,
          "DSEx.Predict.Search.run/3 expects a candidate list and an arity-2 evaluator, got: " <>
  ...
  defp run_sequential(indexed, evaluator, outcomes, timeout) do
    task = DSEx.Tasks.async_nolink(fn -> evaluate(indexed, evaluator, outcomes) end)
      ...
      try do
        case evaluator.(candidate, %{
          ...
          {:ok, value, metric_result} ->
            metric_result = DSEx.Metrics.normalize_result(metric_result)
