defmodule DSEx.Evaluate.SemanticF1 do
  ...
  precision and recall, while DSEx clamps both values and computes the harmonic
  ...
  def new(opts \\ []) do
    ...
    metric_opts = DSEx.Options.validate!(metric_opts, @metric_options, "SemanticF1.new/1")
    decompositional = metric_opts[:decompositional]

    %__MODULE__{
      predict: DSEx.Predict.ChainOfThought.new(signature(decompositional), predict_opts),
  ...
  def call(%__MODULE__{predict: predict} = evaluator, inputs) do
    with {:ok, judge_inputs, trace} <- semantic_inputs(inputs),
         {:ok, judgment} <- DSEx.Predict.ChainOfThought.call(predict, judge_inputs),
         {:ok, score} <-
           f1_score(
             DSEx.Prediction.get(judgment, :precision),
             DSEx.Prediction.get(judgment, :recall)
           ) do
      result = if(is_nil(trace), do: score, else: score >= evaluator.threshold)

      {:ok,
       judgment
       |> DSEx.Prediction.put(:f1, score)
       |> DSEx.Prediction.put(:score, result)
  ...
  defp value(%DSEx.Example{} = example, key), do: value(DSEx.Example.to_map(example), key)
  defp value(%DSEx.Prediction{} = prediction, key), do: fetch(prediction.fields, key)
...
defmodule DSEx.Evaluate.CompleteAndGrounded do
  ...
  the same clamped harmonic mean used by `DSEx.Evaluate.SemanticF1`.
  """
  ...
  def new(opts \\ []) do
    ...
    metric_opts =
      DSEx.Options.validate!(metric_opts, @metric_options, "CompleteAndGrounded.new/1")
  ...
  def call(%__MODULE__{} = evaluator, inputs) do
    with {:ok, normalized, trace} <- complete_and_grounded_inputs(inputs),
         {:ok, completeness} <-
           DSEx.Predict.ChainOfThought.call(evaluator.completeness, %{
             question: normalized.question,
             ground_truth: normalized.ground_truth,
             system_response: normalized.system_response
           }),
         {:ok, groundedness} <-
           DSEx.Predict.ChainOfThought.call(evaluator.groundedness, %{
             question: normalized.question,
             retrieved_context: normalized.retrieved_context,
             system_response: normalized.system_response
           }),
         {:ok, score} <-
           DSEx.Evaluate.SemanticF1.f1_score(
             DSEx.Prediction.get(groundedness, :groundedness),
             DSEx.Prediction.get(completeness, :completeness)
           ) do
      result = if(is_nil(trace), do: score, else: score >= evaluator.threshold)
      ...
      fields =
        ...
        |> Map.merge(DSEx.Prediction.to_map(groundedness))
        |> Map.put(:f1, score)
        |> Map.put(:score, result)

      {:ok, DSEx.Prediction.new(fields, score: result)}
    end
  end
