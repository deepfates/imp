defmodule Dachshund.Metrics do
  ...
  def exact_match(field \\ :answer) do
    fn example, prediction ->
      normalize_text(Dachshund.Example.get(example, field)) ==
        normalize_text(Dachshund.Prediction.get(prediction, field))
    end
  end

  def answer_passage_match(answer_field \\ :answer, context_field \\ :context) do
    fn example, prediction ->
      answer = normalize_text(Dachshund.Example.get(example, answer_field))
      context = normalize_text(Dachshund.Prediction.get(prediction, context_field, ""))
