defmodule Dachshund.MCP do
  @moduledoc """
  MCP-style tool catalog importer.

  `Dachshund.MCP.import_tools/1` converts either an in-process catalog or a
  transport-backed HTTP catalog into ordinary `Dachshund.Tool` values. Imported tools
    ...
    defp attach_remote_run(client, tool) do
      ...
      Map.put(tool, "run", fn arguments ->
        ...
        with {:ok, %{status: status, body: response}} when status in 200..299 <-
               Dachshund.HTTP.post(client.transport, client.url, headers, Jason.encode!(body), []),
  ...
  @doc "Imports a catalog or list of tool schemas into `Dachshund.Tool` structs."
