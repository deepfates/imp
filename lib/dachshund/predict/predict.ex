defmodule Dachshund.Predict.Predict do
  @moduledoc "Basic Dachshund module that maps signature inputs to outputs with an LM."
  ...
  def new(signature, opts \\ []) do
    settings = Dachshund.Settings.get()
  ...
  def call(%__MODULE__{} = predict, inputs) when is_list(inputs) or is_map(inputs) do
    with {:ok, lm} <- require_lm(predict.lm),
         ...
         {:ok, raw} <- Dachshund.LM.generate(lm, messages, predict.config),
  ...
  def with_signature(%__MODULE__{} = predict, signature),
    do: %{predict | signature: Dachshund.Signature.ensure(signature)}

  def dump(%__MODULE__{} = predict) do
    %{
      "signature" => Dachshund.Signature.dump(predict.signature),
      "demos" => Enum.map(predict.demos, &Dachshund.Example.to_map/1),
  ...
  defp dump_lm(%Dachshund.Clients.HTTPLM{} = lm), do: Dachshund.Clients.HTTPLM.dump(lm)
  ...
  defp add_trace(%Dachshund.Prediction{} = prediction, messages, raw) do
