defmodule Dachshund.Predict.RLM do
  @moduledoc """
  ...
  - `:tools` - list of `Dachshund.Tool` values available to `tool` actions.
  ...
  def new(signature, opts \\ []) do
    ...
    %__MODULE__{
      ...
      adapter: Keyword.get(opts, :adapter, Dachshund.Adapter.Chat),
  ...
  `llm_query`, and `submit`. A successful submit returns a `Dachshund.Prediction`
  ...
  defp controller_action(%__MODULE__{} = rlm, state, iteration) do
    messages = [
      ...
      %{
        ...
        content:
          Jason.encode!(%{
            signature: Dachshund.Signature.to_spec(rlm.signature),
            iteration: iteration,
    ...
    Dachshund.LM.generate(rlm.lm, messages, [])
  end
  ...
  defp step(rlm, %{"action" => "submit", "result" => result}, state, iteration)
    ...
    case Dachshund.Adapter.Chat.parse(rlm.signature, result, []) do
      {:ok, prediction} ->
  ...
  defp step(rlm, %{"action" => "eval", "code" => code}, state, iteration) when is_binary(code) do
    observation =
      code
      |> Dachshund.Sandbox.eval(state.vars)
  ...
  defp step(%__MODULE__{} = rlm, %{"action" => "llm_query"} = action, state, iteration) do
    if state.llm_calls >= rlm.max_llm_calls do
      ...
      signature = Map.get(action, "signature", Dachshund.Signature.to_spec(rlm.signature))
      inputs = Map.get(action, "inputs", %{})
      program = Dachshund.Predict.Predict.new(signature, lm: rlm.sub_lm, adapter: rlm.adapter)
      result = Dachshund.Predict.Predict.call(program, inputs)
  ...
  defp add_trace(%Dachshund.Prediction{} = prediction, state) do
