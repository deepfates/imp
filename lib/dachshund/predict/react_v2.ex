defmodule Dachshund.Predict.ReActV2 do
  ...
  def new(signature, tools, opts \\ []) do
    ...
    submit = Dachshund.Tool.new(:submit, "Submit final outputs", fn args -> args end)
    tools = Map.put(tool_map, :submit, submit)

    react_signature = %Dachshund.Signature{
      inputs:
        signature.inputs ++
          [
            Dachshund.Signature.Field.new(:history, :input),
            Dachshund.Signature.Field.new(:tools, :input)
          ],
      outputs: [
        ...
        Dachshund.Signature.Field.new(:tool_calls, :output)
      ],
    ...
    %__MODULE__{
      signature: signature,
      react: Dachshund.Predict.Predict.new(react_signature, opts),
      tools: tools,
  ...
  defp run_loop(_agent, _inputs, history, 0) do
    {:ok, Dachshund.Prediction.new(%{history: history, termination_reason: :max_iters})}
  end
  ...
  defp run_loop(agent, inputs, history, remaining) do
    ...
    with {:ok, prediction} <- Dachshund.Predict.Predict.call(agent.react, call_inputs) do
      case Dachshund.Prediction.get(prediction, :tool_calls, []) do
        [] ->
        ...
        calls ->
          ...
          cond do
            ...
            final ->
              ...
              case Dachshund.Adapter.Chat.parse(agent.signature, final, []) do
                {:ok, prediction} ->
                  prediction =
                    prediction
                    |> Dachshund.Prediction.put(:history, history)
                    |> Dachshund.Prediction.put(:termination_reason, :submit)
    ...
    Enum.reduce(calls, {[], nil}, fn call, {events, final} ->
      ...
      result =
        cond do
          ...
          true -> Dachshund.Tool.call(tool, args)
        end
  ...
  defp project_outputs(signature, prediction) do
    fields =
      Map.take(
        ...
        Dachshund.Signature.output_names(signature)
      )

    Dachshund.Prediction.new(fields, metadata: prediction.metadata)
  end

  defp coerce_tool(%Dachshund.Tool{} = tool), do: tool
