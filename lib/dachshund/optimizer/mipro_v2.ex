defmodule Dachshund.Optimizer.MIPROv2 do
  @moduledoc "Joint instruction and demonstration search inspired by Dachshund's MIPROv2."
  ...
  def compile(%__MODULE__{} = optimizer, program, trainset, devset) do
    instructions = Dachshund.Optimizer.InstructionSearch.candidate_instructions(program, trainset)
    demo_sets = demo_candidates(trainset, optimizer.demos_per_candidate)
    evaluator = Dachshund.Evaluate.new(devset, optimizer.metric)

    baseline =
      {Dachshund.Evaluate.run(evaluator, program).score, program, %{trial: 0, baseline: true}}
  ...
  defp build_candidate(program, instruction, demos, k) do
    %Dachshund.Optimizer.LabeledFewShot{k: k}
    |> Dachshund.Optimizer.LabeledFewShot.compile(
      Dachshund.Optimizer.InstructionSearch.put_instruction(program, instruction),
      demos
    )
  end
end
