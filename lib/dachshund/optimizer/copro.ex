defmodule Dachshund.Optimizer.COPRO do
  ...
  def compile(%__MODULE__{} = optimizer, program, trainset, devset) do
    {compiled, round_reports} =
      ...
      |> Enum.reduce({program, []}, fn round, {current, reports} ->
        candidates =
          current
          |> Dachshund.Optimizer.InstructionSearch.candidate_instructions(trainset,
        ...
        report = Dachshund.Optimizer.Report.fetch(next)
