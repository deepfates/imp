defmodule Dachshund.Optimizer.BetterTogether do
  ...
  def new(metric, optimizers \\ %{}) do
    optimizers =
      if map_size(Map.new(optimizers)) == 0 do
        %{
          p: Dachshund.Optimizer.RandomSearch.new(metric),
          w: Dachshund.Optimizer.BootstrapFinetune.new(metric)
        }
      else
        Map.new(optimizers)
      end
  ...
  defp compile_step(
         %Dachshund.Optimizer.BootstrapFinetune{} = optimizer,
         program,
         trainset,
         _valset
       ) do
    case Dachshund.Optimizer.BootstrapFinetune.compile(optimizer, program, trainset) do
