defmodule Dachshund.Optimizer.InstructionSearch do
  @moduledoc false
  ...
  def compile(program, metric, trainset, devset, candidates, opts \\ []) do
    ...
    results =
      ...
      |> Enum.map(fn instruction ->
        ...
        {Dachshund.Evaluate.run(evaluator, candidate).score, candidate, instruction}
      end)
      |> Kernel.++([
        {Dachshund.Evaluate.run(evaluator, program).score, program, current_instruction(program)}
      ])
  ...
  def put_instruction(%Dachshund.Predict.Predict{signature: signature} = program, instruction) do
    Dachshund.Predict.Predict.with_signature(program, %{signature | instructions: instruction})
  end

  def put_instruction(%Dachshund.Predict.ChainOfThought{predict: predict} = program, instruction) do
    %{program | predict: put_instruction(predict, instruction)}
  end

  def put_instruction(program, _instruction), do: program

  def current_instruction(%Dachshund.Predict.Predict{signature: signature}),
    do: signature.instructions

  def current_instruction(%Dachshund.Predict.ChainOfThought{predict: predict}),
  ...
  defp infer_labels(trainset) do
    ...
    |> Enum.flat_map(fn example ->
      example |> Dachshund.Example.labels() |> Dachshund.Example.to_map() |> Map.keys()
    end)
    |> Enum.uniq()
  ...
  defp maybe_put_demos(%Dachshund.Predict.Predict{} = program, demos),
    do: Dachshund.Predict.Predict.with_demos(program, demos)

  defp maybe_put_demos(%Dachshund.Predict.ChainOfThought{predict: predict} = program, demos) do
    %{program | predict: Dachshund.Predict.Predict.with_demos(predict, demos)}
  end

  defp maybe_put_demos(program, _demos), do: program

  defp attach_optimizer_metadata(%Dachshund.Predict.Predict{} = program, metadata),
    do: %{program | metadata: Map.merge(program.metadata, metadata)}

  defp attach_optimizer_metadata(
         %Dachshund.Predict.ChainOfThought{predict: predict} = program,
         metadata
       ) do
