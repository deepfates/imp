defmodule Dachshund.Optimizer.SIMBA do
  ...
  def compile(%__MODULE__{} = optimizer, program, trainset, devset) do
    ...
    initial = {Dachshund.Evaluate.run(evaluator, program).score, program}
    ...
    {best_score, best_program, candidates} =
      ...
      |> Enum.reduce({elem(initial, 0), elem(initial, 1), []}, fn step,
        ...
        instruction =
          (Dachshund.Optimizer.InstructionSearch.current_instruction(best_program) || "") <>
            "\nPrefer answers that score well on the metric."

        candidate =
          %Dachshund.Optimizer.LabeledFewShot{k: optimizer.demos_per_step}
          |> Dachshund.Optimizer.LabeledFewShot.compile(
            Dachshund.Optimizer.InstructionSearch.put_instruction(best_program, instruction),
            demos
          )

        score = Dachshund.Evaluate.run(evaluator, candidate).score
