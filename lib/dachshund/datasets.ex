defmodule Dachshund.Datasets do
  ...
  def from_records(records, input_keys, opts \\ []) do
    ...
    |> Enum.map(fn {record, index} ->
      ...
      |> Dachshund.Example.new()
      |> Dachshund.Example.with_inputs(input_keys)
    end)
  end
  ...
  def gsm8k(path), do: jsonl(path, [:question], record: Dachshund.Datasets.GSM8K.Record)

  def hotpotqa(path),
    do: jsonl(path, [:question, :context], record: Dachshund.Datasets.HotPotQA.Record)
...
defmodule Dachshund.Datasets.DataLoader do
  ...
  def load(path, input_keys, opts \\ []) do
    case Keyword.get(opts, :format, Path.extname(path)) do
      ".csv" -> Dachshund.Datasets.csv(path, input_keys)
      _ -> Dachshund.Datasets.jsonl(path, input_keys)
    end
  end
end

defmodule Dachshund.Datasets.GSM8K do
  @moduledoc "GSM8K-style JSONL dataset loader."
  defmodule Record, do: defstruct([:question, :answer])

  def load(path), do: Dachshund.Datasets.gsm8k(path)

  def metric(example, prediction, _trace \\ nil) do
    Dachshund.Metrics.em(
      Dachshund.Prediction.get(prediction, :answer),
      Dachshund.Example.get(example, :answer)
    )
  end
end

defmodule Dachshund.Datasets.HotPotQA do
  @moduledoc "HotPotQA-style JSONL dataset loader."
  defmodule Record, do: defstruct([:question, :context, :answer])

  def load(path), do: Dachshund.Datasets.hotpotqa(path)
end

defmodule Dachshund.Datasets.MATH do
  @moduledoc "MATH-style JSONL dataset loader."
  defmodule Record, do: defstruct([:problem, :solution, :answer])

  def load(path), do: Dachshund.Datasets.jsonl(path, [:problem], record: __MODULE__.Record)
end

defmodule Dachshund.Datasets.Colors do
  @moduledoc "Simple color dataset helper."
  defmodule Record, do: defstruct([:input, :label])

  def load(records),
    do: Dachshund.Datasets.from_records(records, [:input], record: __MODULE__.Record)
end
