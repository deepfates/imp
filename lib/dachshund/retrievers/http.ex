defmodule Dachshund.Retrievers.HTTP do
  ...
  def retrieve(%__MODULE__{} = retriever, query, opts \\ []) do
    ...
    with {:ok, %{status: status, body: response}} when status in 200..299 <-
           Dachshund.HTTP.post(retriever.transport, retriever.url, headers, body, opts),
...
defmodule Dachshund.Retrievers.Databricks do
  @moduledoc "Databricks Vector Search retriever."

  def new(endpoint_url, opts \\ []) do
    Dachshund.Retrievers.HTTP.new(endpoint_url,
      transport: Keyword.get(opts, :transport, Dachshund.HTTP.Hackneyless),
