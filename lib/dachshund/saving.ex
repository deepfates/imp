defmodule Dachshund.Saving do
  ...
  def dump(%Dachshund.Predict.Predict{} = program),
    do: Map.put(Dachshund.Predict.Predict.dump(program), "type", "predict")

  def dump(%Dachshund.Predict.ChainOfThought{predict: predict}) do
  ...
  def load(
    ...
    Dachshund.Predict.Predict.new(Dachshund.Signature.load(signature),
      demos: Enum.map(demos, &Dachshund.Example.new/1),
  ...
  def load(%{"type" => type}) do
    raise ArgumentError, "unsupported saved Dachshund program type: #{inspect(type)}"
  end
  ...
  defp decode_adapter(nil), do: Dachshund.Adapter.Chat
  ...
  defp decode_lm(%{"provider" => provider, "model" => model} = state) do
    ...
    Dachshund.Clients.HTTPLM.new(model,
      api_key: nil,
      provider: provider,
  ...
  defp decode_provider(provider) do
    case to_string(provider) do
      ...
      other -> raise ArgumentError, "unsupported saved Dachshund provider: #{inspect(other)}"
    end
  end
end
