defmodule Dachshund.Clients.HTTPLM do
  ...
  def stream(%__MODULE__{} = lm, messages, opts \\ []) do
    ...
    lm.transport
    |> Dachshund.HTTP.stream(endpoint(lm), headers, body, request_opts)
  ...
  defp post_with_retries(lm, headers, body, request_opts, %{
    ...
    Enum.reduce_while(1..attempts, nil, fn attempt, _last ->
      case Dachshund.HTTP.post(lm.transport, endpoint(lm), headers, body, request_opts) do
  ...
  defp encode_message(%{role: role, content: content}) when is_list(content),
    do: %{role: role_name(role), content: Dachshund.Adapters.Types.content_to_openai(content)}
  ...
  defp parse_stream_chunk({:error, reason}),
    do: [%Dachshund.Streaming.Messages.StreamResponse{chunk: {:error, reason}, done: true}]
  ...
  defp parse_sse_line("data: [DONE]"),
    do: [%Dachshund.Streaming.Messages.StreamResponse{done: true}]
      ...
      chunks =
        cond do
          is_binary(delta["content"]) ->
            [%Dachshund.Streaming.Messages.StreamResponse{chunk: delta["content"]}]

          is_list(delta["tool_calls"]) ->
            [
              %Dachshund.Streaming.Messages.StreamResponse{
  ...
  defp parse_stream_event(%{"output_text" => text}),
    do: [%Dachshund.Streaming.Messages.StreamResponse{chunk: text}]
