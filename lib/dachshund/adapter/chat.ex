defmodule Dachshund.Adapter.Chat do
  ...
  @impl true
  def parse(_signature, %Dachshund.Prediction{} = prediction, _opts), do: {:ok, prediction}
  ...
  def parse(signature, text, _opts) when is_binary(text) do
    outputs = Dachshund.Signature.output_names(signature)
  ...
  defp build_prediction(signature, fields) do
    ...
    output_names = Dachshund.Signature.output_names(signature)
    ...
    if missing == [] do
      {:ok, Dachshund.Prediction.new(coerce_fields(signature, Map.take(fields, output_names)))}
    else
  ...
  defp parse_labelled_text(signature, text) do
    allowed =
      signature
      |> Dachshund.Signature.output_names()
