defmodule Dachshund.Streaming do
  @moduledoc "Enumerable-friendly streaming helpers."

  def stream(program, inputs, opts \\ []) do
    cond do
      Keyword.get(opts, :provider_stream, false) and match?(%Dachshund.Predict.Predict{}, program) ->
  ...
  defp provider_stream(%Dachshund.Predict.Predict{} = program, inputs, opts) do
    inputs = Map.new(inputs)
    messages = program.adapter.format(program.signature, inputs, demos: program.demos)

    program.lm
    |> Dachshund.Clients.HTTPLM.stream(
      messages,
  ...
  defp call_once(program, inputs) do
    case program.__struct__.call(program, inputs) do
      {:ok, prediction} ->
        {:ok, prediction |> Dachshund.Prediction.to_map() |> Map.values() |> Enum.join("")}
