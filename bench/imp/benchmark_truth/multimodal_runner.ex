defmodule Imp.BenchmarkTruth.MultimodalRunner do
  ...
  @request_audit_message :imp_multimodal_serialized_request_audit
  @response_audit_message :imp_multimodal_provider_response_audit
  ...
  defp do_execute_sample(sample, context, started) do
    ...
    lm = Imp.req_llm(provider["req_llm_model"], lm_opts)
    messages = [%{role: :user, content: [attachment, sample["prompt"]]}]

    result =
      Imp.Clients.ReqLLM.generate(lm, messages, generation_opts(provider["generation"]))
  ...
  defp install_audit_plugin(client_opts, owner, sample_id, provider) do
    ...
    audit_plugin = fn request ->
      ...
      |> Req.Request.append_request_steps(
        imp_multimodal_serialized_request_audit:
          {__MODULE__, :audit_serialized_request, [owner, sample_id, dependency]}
      )
      |> Req.Request.append_response_steps(
        imp_multimodal_provider_response_audit:
  ...
  defp unwrap_response(%{__imp_lm_metadata__: _metadata} = result) do
    with {:ok, output, metadata} <- Imp.LM.Result.split(result),
  ...
  defp report(manifest, rows, mode, max_concurrency, checkpoint_path, run_stats) do
    ...
    %{
      "artifact_schema" => "imp.multimodal_quality.v2",
      ...
      "rows" => rows,
      "runner" => %{
        "dispatch" => "Imp.Clients.ReqLLM.generate/3",
        "max_concurrency" => max_concurrency,
  ...
  defp redact_failure_text(text, nil), do: Imp.Redaction.redact(text)
