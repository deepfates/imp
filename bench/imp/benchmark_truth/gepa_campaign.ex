defmodule Imp.BenchmarkTruth.GepaCampaign do
  ...
  alias Imp.BenchmarkTruth.GepaCampaignBudget
  ...
  def run(opts) do
    ...
    try do
      ...
      validate_imp_rows!(rows, partial?: partial?)
  ...
  defp validate_source_commits!(source_commits) do
    unless Imp.BenchmarkTruth.GepaReplicationContract.valid_source_commits?(source_commits) do
      raise ArgumentError,
            "source_commits must contain concrete dspy, imp, and gepa_artifact identities"
    end
  end
  ...
  defp validate_requested_families!(families, specs) do
    ...
    cond do
      families == [] ->
        raise ArgumentError, "Imp GEPA campaign requires at least one family"

      unknown != [] ->
        raise ArgumentError, "unknown Imp GEPA campaign families: #{Enum.join(unknown, ", ")}"

      missing_specs != [] ->
        raise ArgumentError,
              "Imp GEPA dataset root missing requested families: #{Enum.join(missing_specs, ", ")}"
  ...
  defp validate_imp_rows!(rows, opts) do
    families = Enum.map(rows, & &1["family"])
    missing = @required_families -- families

    if not Keyword.get(opts, :partial?, false) and missing != [] do
      raise ArgumentError, "Imp GEPA campaign rows missing families: #{Enum.join(missing, ", ")}"
    end

    Enum.each(rows, fn row ->
      unless is_map(get_in(row, ["results", "imp_gepa"])) do
        raise ArgumentError,
              "Imp GEPA campaign row missing results.imp_gepa for #{row["family"]}"
      end
      ...
      unless row["evidence_level"] in ["research_campaign", "research_preflight"] and
        ...
        raise ArgumentError,
              "Imp GEPA campaign row missing research metadata for #{row["family"]}"
      end
      ...
      if row["family"] in ["HotpotQABench", "hoverBench"] do
        ...
        unless hover_retrieval_provenance?(retrieval) do
          raise ArgumentError,
                "Imp GEPA #{row["family"]} row requires source-exact BM25/wiki retrieval provenance"
        end
      end
    end)
  ...
  defp row(spec, context, token_cost) do
    ...
    try do
      ...
      %{
        ...
        "dataset" => %{
          "source" => "Imp GEPA dataset root #{Path.expand(dataset_root)}",
        ...
        "seed_selection" => %{
          "imp_gepa" => %{
            ...
            "source" => "Imp GEPA campaign completed-seed dev score comparison"
          }
        },
        "train_dev_test_gap" => %{
          "train" => best.train,
        ...
        "results" => %{
          "imp_gepa" => %{
            "score" => best.test,
            "source" => "Imp GEPA campaign runner #{context.source_git_sha} #{family}/#{program}",
  ...
  defp maybe_put_papillon_judge(
    ...
    Map.put(row, "metric_judge", %{
      ...
      "leakage_judge" => "Imp ChainOfThought JudgeLeakage source-faithful pii leaked-count check",
  ...
  defp retrieval_evidence(retrieval, execution) do
    ...
    Map.merge(retrieval, %{
      "verified" => true,
      "implementation" => if(upstream?, do: "upstream_python_bm25s", else: "imp_local_bm25")
    })
  end
  ...
  defp metric_call_evidence(best, seed_results) do
    %{
      ...
      "source" => "Imp.Optimizer.GEPA report metadata backed by Imp.Optimizer.GEPA.Budget",
      "observed" => %{"imp_gepa" => best.optimizer_metric_calls},
      "enforced_limits" => %{"imp_gepa" => true},
      "per_seed" =>
