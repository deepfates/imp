...
defmodule LocalSIMBAFeedbackTREC.Contract do
  ...
  def load!(imp, path) do
    ...
    excluded_source_ids =
      ...
      |> Enum.flat_map(fn exclusion ->
        path = Path.join(imp, relocated(exclusion["path"]))
  ...
  # The sealed contract names sibling experiments by the examples/ paths they
  # had when it was written; they now live in research/.
  defp relocated("examples/local_" <> rest), do: "research/local_" <> rest
  defp relocated(path), do: path
  ...
  def load_treatment!(imp, contract_path, treatment_path) do
    ...
    unless Path.expand(relocated(task_contract["path"]), imp) == Path.expand(contract_path) and
